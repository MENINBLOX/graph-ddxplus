...
def main():
    ...
    chosen_dc = {}
    # v320: multi-prompt ensemble (4 prompts × N=5 each = 20 votes per tied case)
    N_PER_PROMPT = 5
    cot_sampling = SamplingParams(temperature=0.7, max_tokens=400, n=N_PER_PROMPT, seed=42)

    def make_prompts(pd, disease_list):
        # 4 different prompts capturing different reasoning angles
        clinical = f"""You are a clinical reasoner. Use BOTH symptom matching AND clinical context (epidemiologic exposure, age, season, severity) to discriminate among similar diagnoses.
...
PICK: <number>"""
        ...
        ruleout = f"""Patient: {pd['age_sex']}
Chief complaint: {pd['chief']}
{pd['profile']}

For each candidate, identify SUPPORTING evidence the patient has and CONTRADICTING evidence (e.g., features absent that should be present). Pick the candidate with strongest support and weakest contradiction.
...
Format your reply EXACTLY as:
RULE-OUT:
(1) <name>: SUPPORT=<brief>; CONTRADICT=<brief or none>
(2) <name>: SUPPORT=<brief>; CONTRADICT=<brief or none>
...
PICK: <number>"""

        deep = f"""Patient: {pd['age_sex']}
Chief complaint: {pd['chief']}
{pd['profile']}

For each tied candidate, reason in 2 sentences:
- Sentence 1: What patient evidence MATCHES this diagnosis?
- Sentence 2: What evidence CONTRADICTS or is unusual?
Then pick most likely.
...
Format:
ANALYSIS:
(1) MATCHES=<sentence>; CONTRADICTS=<sentence>
(2) MATCHES=<sentence>; CONTRADICTS=<sentence>
...
PICK: <number>"""

        return [clinical, plain, ruleout, deep]

    print(f"\n[Stage 2] Multi-prompt ensemble (4 prompts × {N_PER_PROMPT} samples = {4*N_PER_PROMPT} votes)...", flush=True)
    tb_convs = []; tb_meta = []
    for c_idx, tied_dcs in small_ties:
        ...
        disease_list = "\n".join(lines)
        prompts = make_prompts(pd, disease_list)
        # Build conv list and meta with prompt index
        for p_idx, p in enumerate(prompts):
            tb_convs.append([{"role": "user", "content": p}])
            tb_meta.append((c_idx, tied_dcs, p_idx))

    print(f"  Tie-break LLM calls: {len(tb_convs)} (4 prompts × {len(small_ties)} cases × N={N_PER_PROMPT})", flush=True)
    CHUNK = 5000
    t2 = time.time()
    sample_outs = None
    # Aggregate picks per c_idx across all prompts and samples
    picks_per_case = {}
    for chunk_start in range(0, len(tb_convs), CHUNK):
        ...
        for i, out in enumerate(outs):
            c_idx, tied_dcs, p_idx = tb_meta[chunk_start + i]
            if c_idx not in picks_per_case:
                picks_per_case[c_idx] = (tied_dcs, [])
            for sample in out.outputs:
                ...
                if m:
                    p = int(m.group(1)) - 1
                    if 0 <= p < len(tied_dcs):
                        picks_per_case[c_idx][1].append(p)
        print(f"  Tie-break: {chunk_start+len(chunk)}/{len(tb_convs)} ({time.time()-t2:.0f}초)", flush=True)
    # Final majority vote per case
    from collections import Counter as _C
    for c_idx, (tied_dcs, picks) in picks_per_case.items():
        if picks:
