#!/usr/bin/env python3
"""v71 forensic — RANDOM sample 10 success + 10 failure cases.

Valid SOTA = v71 (strict zero-shot KG-only, no LLM at inference).
Config: top_K=full, cosine + IDF (df_thr=0.12, alpha=1.0, beta=0.75)
        + self-aware negative penalty (tau=2.0, sharp=0.5, lam=0.4)
"""
...
OUT = "docs/forensic_v71_random10x10.md"
...
def main():
    ...
    # Read all patients and compute ranks
    ...
    # RANDOM split — not biased by score
    ...
    print(f"Pool: {len(successes)} success, {len(failures)} failure (rank>=3)")
    print(f"Sampled: {len(suc_sample)} success, {len(fail_sample)} failure", flush=True)

    # Names for involved CUIs
    all_target = set()
    ...
    with open(OUT, "w") as f:
        f.write(f"# v71 Forensic — Random 10 Success + 10 Failure\n\n")
        f.write(f"- Config: strict zero-shot KG-only (cosine + IDF + self-aware neg)\n")
        f.write(f"- Sampling: random 5000 from 134K, then random 10 from each pool\n")
        f.write(f"- Pool sizes: {len(successes)} success (rank=1), {len(failures)} failure (rank≥3)\n\n")
        ...
        def render(c, label):
            ...
            for t in c["ev_text"][:12]:
                f.write(f"- {t}\n")
            if len(c["ev_text"]) > 12:
                f.write(f"- ... +{len(c['ev_text'])-12} more\n")
            f.write(f"\n**Patient CUIs ({len(c['pcuis_in_evs'])}):**\n")
            sorted_pc = sorted(c["pcuis_in_evs"], key=lambda x: -idf.get(x, 1.0))
            for cui in sorted_pc:
        ...
        import statistics
        f.write("\n## Aggregate\n\n")
        f.write(f"- Success: avg pcuis={statistics.mean([len(c['pcuis_in_evs']) for c in suc_sample]):.1f}, "
                f"avg true_score={statistics.mean([c['true_score'] for c in suc_sample]):.4f}\n")
        f.write(f"- Failure: avg pcuis={statistics.mean([len(c['pcuis_in_evs']) for c in fail_sample]):.1f}, "
                f"avg true_score={statistics.mean([c['true_score'] for c in fail_sample]):.4f}\n")
