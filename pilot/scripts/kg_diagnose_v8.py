#!/usr/bin/env python3
"""진단 v8: 강화된 re-ranking (KG 증상 정보 포함).

v7: 환자 증상 + 후보 질환 이름 → LLM re-rank → 48.1%
v8: 환자 증상 + 후보 질환(KG 주요 증상 포함) → LLM re-rank

개선:
  1. 각 후보 질환의 KG 상위 증상을 프롬프트에 포함
  2. LLM이 환자 증상과 질환 증상을 직접 비교 가능
  3. top-10 유지 (top-20은 오히려 하락)
"""
...
RERANK_PROMPT = """Patient symptoms:
{symptoms}

Candidate diseases (with their known symptoms from medical literature):
{candidates}

Based on the patient's symptoms, which disease is MOST LIKELY? Consider how well the patient's symptoms match each disease's known symptom profile.
...
def disease_symptom_summary(dc, ds, cp, top_n=8):
    """질환의 KG 상위 증상을 텍스트로 요약."""
    syms = ds.get(dc, {})
    if not syms:
        return "No known symptoms"
    top = sorted(syms.items(), key=lambda x: -x[1])[:top_n]
    return ", ".join(cp.get(cui, cui) for cui, _ in top)


def main():
    print("=" * 80, flush=True)
    print("진단 v8: KG 증상 포함 re-ranking", flush=True)
    print("=" * 80, flush=True)
    ...
    for p in patients:
        ...
        top_k = [dc for dc, _ in ranked[:10]]
        patient_candidates.append({
            "patient": p,
            "true_dc": tdc,
            "topk": top_k,
        })
        if top_k and top_k[0] == tdc:
            top1_baseline += 1

    in_topk = sum(1 for pc in patient_candidates if pc["true_dc"] in pc["topk"])
    print(f"  Baseline @1={100*top1_baseline/n:.1f}%, 정답 in top10={100*in_topk/n:.1f}% ({time.time()-t0:.0f}s)", flush=True)

    # [4] 프롬프트 생성 (KG 증상 포함)
    print("\n[4] 프롬프트 생성 (KG 증상 포함)...", flush=True)
    prompts = []
    for pc_item in patient_candidates:
        sym_text = patient_symptoms_text(pc_item["patient"]["evidences"], ev_info)
        cand_lines = []
        for i, dc in enumerate(pc_item["topk"]):
            dname = cui2name.get(dc, cp.get(dc, dc))
            dsyms = disease_symptom_summary(dc, ds, cp, top_n=8)
            cand_lines.append(f"{i+1}. {dname} (typical: {dsyms})")
        cands = "\n".join(cand_lines)
    ...
    for pc_item, out in zip(patient_candidates, outputs):
        ...
        if m:
            idx = int(m.group(1)) - 1
            if 0 <= idx < len(pc_item["topk"]):
                reranked = list(pc_item["topk"])
                chosen = reranked.pop(idx)
                reranked.insert(0, chosen)
            else:
                reranked = pc_item["topk"]; pf += 1
        else:
            reranked = pc_item["topk"]; pf += 1
    ...
    nt = len(patient_candidates)
    print(f"\n  Baseline: @1={100*top1_baseline/nt:.1f}%", flush=True)
    print(f"  Re-ranked: @1={100*t1/nt:.1f}% @3={100*t3/nt:.1f}% @5={100*t5/nt:.1f}%", flush=True)
    print(f"  parse_fail={pf:,}", flush=True)

    print(f"\n{'='*80}", flush=True)
    print(f"Baseline @1={100*top1_baseline/nt:.1f}% → Re-ranked @1={100*t1/nt:.1f}%", flush=True)
