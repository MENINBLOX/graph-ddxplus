...
Candidate diagnoses with their KEY distinguishing features:
{candidates}

Based on the patient's specific symptoms and history, which diagnosis BEST matches?
...
def main():
    print("=" * 80, flush=True)
    print("진단 v16: 감별 프롬프트 (각 후보의 고유 증상 표시)", flush=True)
    print("=" * 80, flush=True)
    ...
    for c in candidates:
        ...
        # 각 후보 질환의 고유 증상 (다른 후보에 없는 것) 추출
        topk = c["topk"]
        all_topk_syms = defaultdict(set)
        for dc in topk:
            for s in ds.get(dc, {}):
                all_topk_syms[dc].add(s)

        cand_lines = []
        for i, dc in enumerate(topk):
            dname = cui2name.get(dc, cp.get(dc, dc))
            my_syms = all_topk_syms[dc]
            other_syms = set()
            for dc2 in topk:
                if dc2 != dc: other_syms |= all_topk_syms[dc2]
            unique = my_syms - other_syms
            # Top unique symptoms by count
            unique_ranked = sorted([(s, ds.get(dc, {}).get(s, 0)) for s in unique], key=lambda x: -x[1])[:5]
            if unique_ranked:
                unique_str = ", ".join(cp.get(s, s) for s, _ in unique_ranked)
                cand_lines.append(f"{i+1}. {dname} (distinctive: {unique_str})")
            else:
