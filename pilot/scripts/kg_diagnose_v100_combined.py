#!/usr/bin/env python3
"""진단 v100: v87 framework with combined PubMed + SemMedDB features.

PubMed cooccurrence: top-4 (broad context)
SemMedDB relations: top-4 (typed relations)
Combined: 8 features that span both extraction approaches.
"""
...
SEMMEDDB_FEATURES = Path("pilot/results/semmeddb_disease_features.json")
...
NOISE_GENERIC = {'C0683323', 'C0205171', 'C0027361', 'C0085578', 'C0001861', 'C0011854', 'C0220825',
                 'C0011900', 'C0026538', 'C2698577', 'C1457887', 'C0023172', 'C0036572',
                 'C0085418', 'C0030705', 'C1457868', 'C0205390'}
GENERIC_TERMS_LOWER = {'symptom', 'sign', 'pain', 'patient', 'disease', 'syndrome', 'condition',
                       'symptoms', 'signs and symptoms', 'cessation of life', 'hospitalization',
                       'patients', 'lesion', 'complication', 'disorder', 'finding',
                       'process pathologic', 'pathologic process', 'physical findings',
                       'malignant neoplasms', 'virus', 'bacteria', 'well adult', 'adult'}
...
def main():
    print("="*80, flush=True)
    print("진단 v100: v87 + PubMed top-4 + SemMedDB top-4 combined features", flush=True)
    print("="*80, flush=True)
    ...
    with open(SEMMEDDB_FEATURES) as f: semmeddb_feats = json.load(f)
    ...
    # Build PubMed cooccurrence dict
    ds = defaultdict(dict)
    ...
    # Combined features per disease
    disease_features = {}
    PUBMED_K = 4
    SEMMEDDB_K = 4
    for dc in dcs_list:
        # PubMed top-K
        pubmed_ranked = sorted(ds.get(dc, {}).items(), key=lambda x: -x[1])
        pubmed_names = []; seen = set()
        for cui, cnt in pubmed_ranked:
            n_ = cp.get(cui, cui)
            nl = n_.lower().strip()
            if not nl or nl in seen or nl in GENERIC_TERMS: continue
            if len(nl) < 3 or len(nl) > 50: continue
            seen.add(nl); pubmed_names.append(n_)
            if len(pubmed_names) >= PUBMED_K: break

        # SemMedDB top-K (skip dups with PubMed)
        sm_ranked = sorted(semmeddb_feats.get(dc, {}).items(), key=lambda x: -x[1])
        sm_names = []
        for cui, cnt in sm_ranked:
            if cui in NOISE_GENERIC or cui == dc: continue
            n_ = cp.get(cui, cui)
            nl = n_.lower().strip()
            if not nl or nl in seen or nl in GENERIC_TERMS_LOWER: continue
            if len(nl) < 3 or len(nl) > 60: continue
            seen.add(nl); sm_names.append(n_)
            if len(sm_names) >= SEMMEDDB_K: break

        all_names = pubmed_names + sm_names
        disease_features[dc] = ", ".join(all_names) if all_names else "—"

    print("\n샘플 combined features:")
    for sample in ["URTI", "Influenza", "PSVT", "Sarcoidosis", "Pneumonia"]:
        for dn, info in diseases.items():
            if dn == sample:
                dc = info["cui"]
                print(f"  {sample}: {disease_features[dc]}", flush=True)
                break
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    np.save(f"pilot/results/v100_stage1_{n}.npy", score_matrix)
...
PICK: <number>"""
    ...
    print(f"\n  v100 (PubMed+SemMedDB combined + CoT) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v100 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
