...
def main():
    ...
    def patient_profile(evidences):
        """v300: include explicit negative responses (asked-but-No)."""
        pain_chars=[]; pain_locs=[]; pain_radiations=[]
        pain_intens=None; pain_speed=None; pain_present=False
        symptoms=[]; history=[]; denied=[]
        for ev in evidences:
            parts = ev.split("_@_"); base=parts[0]; value=parts[1] if len(parts)>1 else None
            info = ev_info.get(base, {}); q = info.get("question_en", "")
            val_en_raw = info.get("value_en", {}).get(value, "") if value else ""
            val_en_low = val_en_raw.lower() if val_en_raw else ""
            # Detect explicit negative answer (Y/N binary or N enumeration)
            is_negative = val_en_low in ("na", "nowhere", "n", "no", "none")
            val_en = fix_translation("" if is_negative else val_en_raw)
            q_clean = re.sub(r"Do you |Are you |Have you |Did you |Is your ", "", q).rstrip("?").strip() if q else ""
            if info.get("is_antecedent"):
                if is_negative and q_clean:
                    denied.append(q_clean)
                else:
            ...
            if "douleur" in base or "_dlr" in base:
                ...
                continue
            if is_negative and q_clean:
                denied.append(q_clean); continue
        ...
        if symptoms: out.append("PRESENT: " + "; ".join(symptoms))
        if history: out.append("HISTORY: " + "; ".join(history))
        if denied: out.append("DENIES (asked but answered No): " + "; ".join(denied))
        return "\n".join(out)
    ...
    # v301: SELECTIVE CoT only when stage1 score gap (top1-top2) is small (≤5)
    # gap=0 (tied): 1388 cases, stage1 acc 34.9% — needs CoT
    # gap=5:        566 cases, stage1 acc 61.1% — needs CoT
    # gap=10:      3046 cases, stage1 acc 75.2% — keep stage1 winner
    GAP_THRESHOLD = 5
    TOP5_K = 5
    ...
    for c_idx in range(n):
        ...
        top5_dcs = [dcs_list[ranked[i]] for i in range(min(TOP5_K, len(ranked)))]
        candidates_per_patient.append((c_idx, top5_dcs))
    print(f"  Selective CoT (gap≤{GAP_THRESHOLD}): {len(candidates_per_patient)}/{n} patients", flush=True)
    ...
    chosen_dc = {}
    cot_sampling = SamplingParams(temperature=0, max_tokens=500)

    print(f"\n[Stage 2] Structured feature-match CoT (top-{TOP5_K}, all patients)...", flush=True)
    tb_convs = []; tb_meta = []
    for c_idx, top_dcs in candidates_per_patient:
        ...
        prompt = f"""You are a clinical reasoner. Choose the most likely diagnosis from the {TOP5_K} candidates by matching patient evidence to typical features.
...
For each candidate, count POSITIVE matches (patient confirms) and NEGATIVE matches (patient explicitly denies). Then pick the candidate with the strongest positive support and least contradiction.

Format your reply EXACTLY as:
ANALYSIS:
(1) <name>: confirmed=[brief list]; denied=[brief list]; verdict=strong/moderate/weak
(2) <name>: confirmed=[...]; denied=[...]; verdict=strong/moderate/weak
(3) <name>: confirmed=[...]; denied=[...]; verdict=strong/moderate/weak
(4) <name>: confirmed=[...]; denied=[...]; verdict=strong/moderate/weak
(5) <name>: confirmed=[...]; denied=[...]; verdict=strong/moderate/weak
PICK: <number>"""
        tb_convs.append([{"role": "user", "content": prompt}])
        tb_meta.append((c_idx, top_dcs))
    ...
    # v300: chosen always takes precedence (LLM picks among top-5)
    t1c = t3c = t5c = t10c = 0
    ...
    print(f"\n  v300 structured-match @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v300 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
