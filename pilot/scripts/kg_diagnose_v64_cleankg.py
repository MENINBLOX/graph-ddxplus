#!/usr/bin/env python3
"""진단 v64: KG cleaned (top-50 per disease by IDF*count) + v54 scoring.
...
def main():
    ...
    # v64: KG noise cleaning - cap each disease's KG to top-50 by IDF*count
    print(f"  KG before clean: {sum(len(s) for s in ds.values()):,} edges", flush=True)
    ...
    for dc, syms in ds.items():
        scored = [(s, c, c * idf_clean.get(s, 0)) for s, c in syms.items()]
        scored.sort(key=lambda x: -x[2])
        ds_clean[dc] = {s: c for s, c, _ in scored[:KG_TOP_N]}
    ds = ds_clean
    print(f"  KG after clean (top-{KG_TOP_N}): {sum(len(s) for s in ds.values()):,} edges", flush=True)

    # Re-collect scuis from cleaned KG
    scuis = set()
    for syms in ds.values(): scuis.update(syms.keys())
