#!/usr/bin/env python3
"""v72 — INITIAL_EVIDENCE chief-complaint boost on top of v71.

DDXPlus's INITIAL_EVIDENCE column = the symptom the patient first
reports (chief complaint). Clinically the most important single anchor.

We boost the matching weight of CUIs from this evidence in the patient
vector. Single algorithm preserved (cosine + IDF + top-K + neg + chief).
"""
...
def load_ddxplus(n_max):
    ...
    with open("data/ddxplus/release_test_patients.csv") as f:
        for row in csv.DictReader(f):
            ...
            chief = row.get("INITIAL_EVIDENCE", "")
            chief_cuis = set()
            if chief:
                m = value_cuis.get(chief, {})
                chief_cuis = set(m.get("_question", []))
            pos_pcuis = set()
            answered_binary = set()
            for ev in evs:
                if "_@_" in ev:
            ...
            patients.append((true_cui, pos_pcuis, neg_binary, chief_cuis)); n += 1
    return dcs_list, patients, binary_evs


def score(pos_pcuis, neg_binary, chief_cuis, profile, idf, beta, signal, lam, chief_boost):
    scores = {}
    pat_vec = {e: idf.get(e, 1.0) ** beta for e in pos_pcuis}
    # Boost chief complaint CUIs
    for c in chief_cuis:
        if c in pat_vec:
            pat_vec[c] *= chief_boost
...
def evaluate(profile, idf, beta, signal, patients, all_evs, dcs_list, lam, chief_boost):
    n = c1 = c3 = c5 = c10 = 0; rr = 0.0
    for true_cui, pos_raw, neg_binary, chief_cuis in patients:
        pos = pos_raw & all_evs
        if not pos: continue
        s = score(pos, neg_binary, chief_cuis, profile, idf, beta, signal, lam, chief_boost)
...
def main():
    ...
    ap.add_argument("--chief_sweep", type=str, default="1.0,1.5,2.0,2.5,3.0,4.0,5.0")
    args = ap.parse_args()
    ...
    value_cuis = json.load(open(VALUE_CUIS))
    sig = precompute_signal_v71(profile, value_cuis, binary_evs, idf, args.tau, args.sharp)
    print(f"=== v72 chief boost — N={args.n} (v71 base) ===")
    for cb in args.chief_sweep.split(","):
        cb = float(cb)
        r = evaluate(profile, idf, args.beta, sig, patients, all_evs, dcs_list, args.lam, cb)
        print(f"  chief_boost={cb:.1f}: @1={r['at1']:.2f}% @3={r['at3']:.2f}% "
