#!/usr/bin/env python3
"""진단 v36: 49개 모든 후보 + 임상 features (KG 미사용 baseline 비교).
...
# 모호한 disease 이름 → LLM이 명확히 이해할 수 있는 이름으로
DISEASE_NAME_FIX = {
    ...
    "Acute dystonic reactions": "Acute dystonic reaction (drug-induced)",
...
def disease_full_name(short_name):
    return DISEASE_NAME_FIX.get(short_name, short_name)


# Hand-curated clinical features (medical textbook knowledge)
# Focus on DISTINGUISHING features for differential diagnosis
CLINICAL_FEATURES = {
    "Pericarditis": "pleuritic chest pain better leaning forward; pericardial friction rub; diffuse ST elevation on ECG",
    "Myocarditis": "chest pain after viral illness; dyspnea; fatigue; possible heart failure signs",
    "Stable angina": "predictable exertional chest pain relieved by rest or nitrates",
    "Unstable angina": "new-onset or accelerating chest pain at rest, no enzyme rise",
    "Possible NSTEMI / STEMI": "severe crushing chest pain >20 min radiating to arm/jaw; diaphoresis",
    "PSVT": "abrupt-onset rapid regular palpitations HR 150-220; abrupt termination",
    "Atrial fibrillation": "irregularly irregular pulse; palpitations; older patient",
    "Acute pulmonary edema": "acute severe dyspnea; orthopnea; pink frothy sputum; bilateral crackles",
    "Spontaneous pneumothorax": "sudden unilateral chest pain + dyspnea; decreased breath sounds; tall thin male",
    "Bronchiectasis": "chronic productive cough with large-volume sputum; recurrent infections",
    "Bronchospasm / acute asthma exacerbation": "wheezing; dyspnea; chest tightness; allergic triggers",
    "Bronchitis": "productive cough mucoid/purulent; often after URI; low-grade fever",
    "Acute COPD exacerbation / infection": "increased dyspnea/cough/sputum in known smoker/COPD",
    "Pulmonary embolism": "sudden dyspnea; pleuritic chest pain; tachycardia; calf swelling; recent immobilization",
    "Influenza": "abrupt fever; myalgia; headache; fatigue; dry cough (systemic)",
    "Pneumonia": "productive cough; fever; dyspnea; focal lung signs",
    "Bronchiolitis": "infant <2yo; wheezing; tachypnea; viral",
    "Pulmonary neoplasm": "chronic cough; hemoptysis; weight loss; smoking history",
    "Tuberculosis": "chronic cough >3 weeks; night sweats; weight loss; hemoptysis",
    "Sarcoidosis": "bilateral hilar lymphadenopathy; dyspnea; multi-organ",
    "Whooping cough": "paroxysmal cough with inspiratory whoop; post-tussive vomiting",
    "URTI": "runny nose; sore throat; mild fever; cough; viral common cold; <10 days",
    "Acute rhinosinusitis": "facial pain/pressure (cheek/forehead); purulent nasal discharge >7-10 days",
    "Chronic rhinosinusitis": "nasal obstruction; post-nasal drip lasting >12 weeks",
    "Allergic sinusitis": "nasal congestion; sneezing; itchy/watery eyes; allergic; clear discharge",
    "Acute otitis media": "ear pain; hearing loss; often after URI; fever",
    "Viral pharyngitis": "sore throat without exudate; mild systemic symptoms",
    "Acute laryngitis": "hoarseness; voice change; after URI",
    "Croup": "barky cough; inspiratory stridor; viral; child 6mo-3yr",
    "Larygospasm": "sudden brief inspiratory stridor; airway closure",
    "Epiglottitis": "severe sore throat; drooling; tripod position; high fever",
    "GERD": "heartburn; acid regurgitation; retrosternal burning; worse lying flat",
    "Boerhaave": "severe vomiting followed by chest pain; subcutaneous emphysema",
    "Inguinal hernia": "groin bulge; pain on coughing/straining; reducible mass",
    "Pancreatic neoplasm": "epigastric pain radiating to back; weight loss; jaundice",
    "Cluster headache": "severe unilateral periorbital pain; lacrimation; ptosis; episodic; agitated",
    "Myasthenia gravis": "fluctuating weakness; ptosis; diplopia; worse with use",
    "Guillain-Barré syndrome": "ascending symmetric weakness; areflexia; post-infection 1-3 weeks",
    "Acute dystonic reactions": "muscle spasm/stiffness; tongue protrusion; oculogyric; after antipsychotic <7 days",
    "Localized edema": "swelling at single anatomic site; mechanical/allergic/bite cause",
    "Anaphylaxis": "rapid onset urticaria + airway/circulatory compromise; allergic trigger",
    "SLE": "malar rash; joint pain; photosensitivity; multi-system; female",
    "HIV (initial infection)": "fever; lymphadenopathy; rash; sore throat; mononucleosis-like",
    "Chagas": "cardiac/GI involvement; triatomine bug exposure",
    "Ebola": "hemorrhagic fever; recent endemic travel",
    "Scombroid food poisoning": "rapid flushing; palpitations; nausea after spoiled fish",
    "Spontaneous rib fracture": "chest pain after cough/minor trauma; rib tenderness",
    "Anemia": "fatigue; pallor; dyspnea on exertion; chronic onset",
    "Panic attack": "sudden intense fear; palpitations; dyspnea; peaks <10 min",
}

def clinical_features(short_name):
    return CLINICAL_FEATURES.get(short_name, "")


def main():
    print("=" * 80, flush=True)
    print("진단 v34: hand-curated clinical features re-rank", flush=True)
    print("=" * 80, flush=True)
    ...
    for p in patients:
        ...
        topk = [dc for dc, _ in ranked[:K]]  # noqa - K could be 49
    ...
    for c in candidates:
        ...
        for i, dc in enumerate(c["topk"]):
            short = cui2name.get(dc, dc)
            name = disease_full_name(short)
            cf = clinical_features(short)
            cands_lines.append(f"{i+1}. {name}: {cf}")
        cands = "\n".join(cands_lines)

        prompt = f"""You are an expert clinician. Based on the patient's presentation, choose the MOST LIKELY diagnosis from the candidates.
...
CANDIDATES (with typical features for each):
{cands}

Compare the patient's symptoms to each candidate's typical features. Which diagnosis BEST EXPLAINS the patient's overall presentation?
...
Answer with ONLY the number (1-{len(c['topk'])})."""
    ...
    print(f"  v28 Re-ranked: @1={100*t1/nt:.1f}% @3={100*t3/nt:.1f}% @5={100*t5/nt:.1f}%", flush=True)
    print(f"  parse_fail={pf}", flush=True)
    print("\n" + "=" * 80, flush=True)
    print(f"v28 GTPA@1 = {100*t1/nt:.1f}% (10K subset)", flush=True)
    print("=" * 80, flush=True)
