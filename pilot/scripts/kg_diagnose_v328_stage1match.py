...
def main():
    ...
    print(f"[v327-match] Union: {n_medkg}/{len(disease_features)} have medkg, all have v87. Combined top-{TOP_K_FEATURES*2} per disease.")
    # v327: load categorized profiles for token-matching score
    cat_path = Path("/mnt/medkg/kg/disease_categorized_by_cui.json")
    cat_by_cui = json.load(cat_path.open()) if cat_path.exists() else {}
    print(f"[v327] Categorized profiles: {len(cat_by_cui)} CUIs")
    # Build per-disease context tokens (from EXPOSURE + COMORBIDITY)
    import re as _re
    STOP = {"the","a","an","of","with","in","to","and","or","is","are","be","by","for","at","on","from","as","that","this","more","than","who","may","can","also","not","no"}
    disease_ctx_tokens = {}
    for cui, cats in cat_by_cui.items():
        toks = set()
        for c_label in ["EXPOSURE","COMORBIDITY","SYSTEMIC"]:
            for item in cats.get(c_label, []):
                for tok in _re.findall(r"[a-zA-Z]{4,}", item.lower()):
                    if tok not in STOP: toks.add(tok)
        disease_ctx_tokens[cui] = toks
    ...
    # v328: apply token-match score to ALL stage1 scores (not just tied)
    print("  v328: applying token-match adjustment to stage1 scores...", flush=True)
    MATCH_GAIN = 2.0
    n_adjusted = 0
    for c_idx in range(len(candidates)):
        pd = patient_data[c_idx]
        ptokens = set(_re.findall(r"[a-zA-Z]{4,}", (pd.get("profile","") + " " + pd.get("chief","")).lower()))
        ptokens -= STOP
        for d_idx, dc in enumerate(dcs_list):
            ctx = disease_ctx_tokens.get(dc, set())
            if ctx:
                overlap = len(ctx & ptokens)
                if overlap > 0:
                    score_matrix[c_idx, d_idx] += MATCH_GAIN * overlap
                    n_adjusted += 1
    # re-evaluate stage1 after adjustment
    t1_after = sum(1 for c_idx, c in enumerate(candidates)
                if dcs_list[int(np.argmax(score_matrix[c_idx]))] == c["true_dc"])
    print(f"  Stage 1 after token-match adj @1 = {100*t1_after/n:.2f}% ({n_adjusted:,} adjustments)", flush=True)
    ...
    for chunk_start in range(0, len(tb_convs), CHUNK):
        ...
        for i, out in enumerate(outs):
            ...
            if picks:
                ...
                vote_counts = _C(picks)
                # v327: combine LLM vote + token-match score
                # patient text = HISTORY + chief + profile tokens
                pd = patient_data[c_idx]
                ptokens = set(_re.findall(r"[a-zA-Z]{4,}", (pd.get("profile","") + " " + pd.get("chief","")).lower()))
                ptokens -= STOP
                MATCH_ALPHA = 1.0
                best_pick = None; best_score = -1e9
                for p, vc in vote_counts.items():
                    dc_at_p = tied_dcs[p]
                    ctx_toks = disease_ctx_tokens.get(dc_at_p, set())
                    overlap = len(ctx_toks & ptokens)
                    score = vc + MATCH_ALPHA * overlap
                    if score > best_score:
                        best_score = score; best_pick = p
                chosen_dc[c_idx] = tied_dcs[best_pick]
            else:
