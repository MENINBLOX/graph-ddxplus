#!/usr/bin/env python3
"""진단 v63: v54 + negative evidence (typical features patient is MISSING).
...
def main():
    ...
    for p in patients:
        ...
        candidates.append({"patient": p, "true_dc": tdc, "topk": topk, "patient_cuis": ps})
    ...
    # Compute disease KG signatures for negative evidence
    sym_df = Counter()
    ...
    for dc, syms in ds.items():
        ...
        disease_top_syms[dc] = [s for s, _ in scored[:8]]
    ...
    for c_idx, c in enumerate(candidates):
        ...
        patient_cuis = c.get("patient_cuis", set())

        for d_idx, dc in enumerate(c["topk"]):
            name = disease_full_name(cui2name.get(dc, dc))
            # Compute negative evidence: typical symptoms patient does NOT have
            top_syms = disease_top_syms.get(dc, [])
            absent = [cp.get(s, "") for s in top_syms if s not in patient_cuis][:3]
            absent = [a for a in absent if a]  # filter empties
...
Typical features patient is MISSING for this diagnosis: {absent_str}

How likely is this hypothesis? Consider both what the patient HAS and what they are MISSING. Reply with ONLY a percentage 0-100 (0=impossible, 100=certain)."""
    ...
    print(f"  v50 score-ranked: @1={100*t1/nt:.1f}% @3={100*t3/nt:.1f}% @5={100*t5/nt:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v50 GTPA@1 = {100*t1/nt:.1f}% (SUBSET={nt})", flush=True)
