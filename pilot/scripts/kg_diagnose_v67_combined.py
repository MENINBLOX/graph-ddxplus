#!/usr/bin/env python3
"""진단 v67: combine stage1 (v59) + stage2 (v54-style) scores with α weight.
...
How likely is this hypothesis given the patient's presentation? Reply with ONLY a percentage 0-100 (0=impossible, 100=certain)."""
    ...
    print("\n[7] Stage 2: v54-style re-score top-10", flush=True)
    convs2 = []
    pair_meta2 = []
    for c_idx, c in enumerate(candidates):
        p = c["patient"]
...
Among ALL possible diagnoses, this is one of the top-10 most likely. How likely is THIS specific hypothesis given the patient? Reply with ONLY a percentage 0-100."""
    ...
    # v67: try multiple combinations of stage1 + stage2 scores
    print("\n[8] v67: combine stage1 + stage2 scores", flush=True)
    for alpha in [0.0, 0.3, 0.5, 0.7, 1.0]:
        t1 = t3 = t5 = 0
        for c_idx, c in enumerate(candidates):
            scored = []
            for d_idx in range(len(stage1_top10[c_idx])):
                # Get stage 1 score (need to find d_idx in the all_scores via topk index)
                dc = stage1_top10[c_idx][d_idx]
                # Find the original index of this dc in candidates[c_idx]["topk"]
                orig_idx = c["topk"].index(dc)
                s1_score = all_scores.get((c_idx, orig_idx), 0)
                s2_score = s2_scores.get((c_idx, d_idx), 0)
                combined = alpha * s2_score + (1 - alpha) * s1_score
                scored.append((d_idx, combined))
            scored.sort(key=lambda x: -x[1])
        ...
        print(f"  α={alpha:>3.1f} (s1=({1-alpha}), s2=({alpha})): @1={100*t1/nt:.1f}% @3={100*t3/nt:.1f}% @5={100*t5/nt:.1f}%", flush=True)

    print(f"\n  Baseline @1={100*baseline/nt:.1f}%", flush=True)
    print(f"  v59 stage1 @1={100*t1_v59/nt:.1f}%, top10={100*in_top10_v59/nt:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v67 GTPA@1 = (see above) (SUBSET={nt})", flush=True)
