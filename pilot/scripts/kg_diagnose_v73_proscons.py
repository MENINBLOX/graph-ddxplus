#!/usr/bin/env python3
"""진단 v73: pros/cons CoT decomposition with strict SCORE: parsing.

각 (patient, top5 candidate) 쌍 → "supporting features", "contradicting features",
"SCORE: NNN" → SCORE 추출. 변별력↑ 의도.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v73: pros/cons CoT scoring", flush=True)
    print("="*80, flush=True)
    ...
    # Stage 1: per-candidate quick score (49 candidates)
...
How likely is this hypothesis given the patient's presentation? Reply with ONLY a percentage 0-100 (0=impossible, 100=certain)."""
    ...
    in_top5 = 0
    for c_idx, c in enumerate(candidates):
        idxs = np.argsort(-score_matrix[c_idx])[:5]
        top5 = [dcs_list[i] for i in idxs]
        stage1_top5.append(top5)
        if top5[0] == c["true_dc"]: t1_v59 += 1
        if c["true_dc"] in top5: in_top5 += 1
    print(f"  v59 stage1 @1={100*t1_v59/n:.1f}%, top5={100*in_top5/n:.1f}%", flush=True)

    # Stage 2: pros/cons CoT scoring on top-5
    print("\n[5] Stage 2: pros/cons CoT on top-5...", flush=True)
    sampling2 = SamplingParams(temperature=0, max_tokens=200, stop=["</score>"])
    convs2 = []
    pair_meta2 = []
    for c_idx, c in enumerate(candidates):
        p = c["patient"]
        ...
        chief = re.sub(r"Do you |Have you |Are you ", "", chief).rstrip("?").strip()
        age_sex = f"{p['age']}yo {'Male' if p['sex']=='M' else 'Female'}"
        for d_idx, dc in enumerate(stage1_top5[c_idx]):
            name = disease_full_name(cui2name.get(dc, dc))
            prompt = f"""Patient: {age_sex}
Chief complaint: {chief}
{profile}
...
Analyze briefly:
SUPPORT: (1-2 features supporting this diagnosis, or "none")
AGAINST: (1-2 features contradicting this diagnosis, or "none")
SCORE: <score>NN</score>

where NN is 0-100 (0=impossible, 100=certain). End with </score>."""
    ...
    for chunk_start in range(0, len(convs2), CHUNK):
        ...
        for i, out in enumerate(outs):
            ...
            text = out.outputs[0].text.strip()
            # Strict parse: <score>NN</score> or "SCORE: NN"
            m = re.search(r"<score>\s*(\d+)\s*</score>", text)
            if not m: m = re.search(r"SCORE:?\s*(\d+)", text, re.IGNORECASE)
            if not m: m = re.search(r"(\d+)\s*$", text)  # last digit
            score = int(m.group(1)) if m else 0
            score = min(score, 100)
    ...
    # show first sample
    if outs: print(f"  샘플 응답: {outs[0].outputs[0].text.strip()[:300]}", flush=True)

    # Pick top-1 from stage 2 scores
    t1 = t3 = t5 = 0
    for c_idx, c in enumerate(candidates):
        scored = [(d_idx, s2_scores.get((c_idx, d_idx), 0)) for d_idx in range(len(stage1_top5[c_idx]))]
        scored.sort(key=lambda x: -x[1])
        reranked = [stage1_top5[c_idx][d_idx] for d_idx, _ in scored]
        tdc = c["true_dc"]
        if reranked[0] == tdc: t1 += 1
        if tdc in reranked[:3]: t3 += 1
        if tdc in reranked[:5]: t5 += 1

    print(f"\n  v59 stage1 @1={100*t1_v59/n:.1f}%, top5={100*in_top5/n:.1f}%", flush=True)
    print(f"  v73 pros/cons @1={100*t1/n:.1f}% @3={100*t3/n:.1f}% @5={100*t5/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v73 GTPA@1 = {100*t1/n:.1f}% (SUBSET={n})", flush=True)
