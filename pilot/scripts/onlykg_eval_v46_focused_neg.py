#!/usr/bin/env python3
"""v46: Focused Negative Evidence — asymmetric yes/no logic.

Key architectural insight: DDXPlus EVIDENCES lists only POSITIVE answers.
Missing evidence = patient answered NO (questionnaire is complete).

Current scoring: pos-only weighting, neg as soft top-K penalty.
New: For each phen P that disease D STRONGLY expects (high weight in KG):
- patient said YES (P in pcuis) → +
- patient said NO (P not in pcuis) → STRONG -
- this directly captures "rule out by absence" reasoning

Combined: 3-channel scoring (current) + focused-negative (new) channel.
"""
...
def main():
    ...
    # NEW: focused negative channel
    ap.add_argument("--w_focused_neg", type=float, default=0.3)
    ap.add_argument("--fneg_top_k", type=int, default=20, help="top-K disease phens to check for negative")
    ap.add_argument("--fneg_weight_threshold", type=float, default=0.5, help="only count phens with weight > threshold")
    args = ap.parse_args()
    ...
    # FOCUSED NEGATIVE: per disease, top-K phens by raw weight (not IDF) — "cardinal" features
    d_fneg = {}
    for d in dcs_list:
        qp = d_q.get(d, {})
        # Top phens that disease STRONGLY expects
        d_fneg[d] = [(p, w) for p, w in sorted(qp.items(), key=lambda x: -x[1])[:args.fneg_top_k] if w > args.fneg_weight_threshold]

    disease_full_phens = {d: set() for d in dcs_list}
    for d in dcs_list:
        for _, p, ed in G.out_edges(d, data=True):
            if ed.get("etype") == "HAS_PHENOTYPE":
                disease_full_phens[d].add(p)
    ...
    with open("data/ddxplus/release_test_patients.csv") as f:
        for row in csv.DictReader(f):
            ...
            pcuis, compound_targets = get_pcuis(evs)
            identity_diseases = pcuis & dcs_set

            s1_scores = {}; cov_scores = {}; prcov_scores = {}; comp_scores = {}; fneg_scores = {}
            for d in dcs_list:
                qp = d_q_idf.get(d, {})
                ...
                if compound_targets and disease_full_phens[d]:
                    ...
                    comp_score = sum(compound_idf.get(c, 0) for c in matched)
                comp_scores[d] = comp_score

                # FOCUSED NEGATIVE: for each "cardinal" phen of D, check if patient has it
                # Penalize disease per missing cardinal phen (heavier than current core_k)
                fneg = 0
                for p, w in d_fneg[d]:
                    if p not in pcuis:
                        # missing cardinal feature — strong evidence against D
                        fneg -= w * idf.get(p, 1.0)
                fneg_scores[d] = fneg
            ...
            fneg_n = normalize_scores(fneg_scores)

            final = {d: args.w_s1*s1_n[d] + args.w_cov*cov_n[d] + args.w_prcov*prcov_n[d] + args.w_compound*comp_n[d] + args.w_focused_neg*fneg_n[d] for d in dcs_list}
    ...
    print(f"v46 [ws1={args.w_s1},wfneg={args.w_focused_neg},fK={args.fneg_top_k},fT={args.fneg_weight_threshold}]: @1={100*c1/n:.2f}% @3={100*c3/n:.2f}% @5={100*c5/n:.2f}% @10={100*c10/n:.2f}% MRR={rr_sum/n:.4f}")
