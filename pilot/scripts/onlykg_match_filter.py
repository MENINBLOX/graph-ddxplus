#!/usr/bin/env python3
"""Filter diseases by minimum match count, then score."""
...
def main():
    ...
    ap.add_argument("--match_count_weight", type=float, default=0.0)
    args = ap.parse_args()
    ...
    with open("data/ddxplus/release_test_patients.csv") as f:
        for row in csv.DictReader(f):
            ...
            for d in dcs_list:
                ...
                n_match = len(matched_phens)
                # Filter: if below threshold AND not identity, severely penalize
                if n_match < args.min_matches and d not in identity_diseases:
                    scores[d] = -1e6
                    continue
                pos = sum(qp[p] for p in matched_phens)
                total = sum(qp.values()) or 1
                s = pos / math.sqrt(total) + args.match_count_weight * n_match
    ...
    print(f"min_matches={args.min_matches} match_count_w={args.match_count_weight} identity={args.identity_boost}:")
