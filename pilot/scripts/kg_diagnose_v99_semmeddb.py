#!/usr/bin/env python3
"""진단 v99: v87 framework with SemMedDB-derived KG features.

Uses SemMedDB AFFECTS/CAUSES/ASSOCIATED_WITH/COEXISTS_WITH/MANIFESTATION_OF
relations instead of raw PubMed co-occurrence. Cleaner relation-typed features.

URTI features:
  PubMed: Urticaria, Rhinitis, Fever, Asthma, Angioedema (corrupted)
  SemMedDB: Coughing, Asthma, Virus, Acute otitis media, Rhinovirus (cleaner)
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v99: v87 + SemMedDB KG features", flush=True)
    print("="*80, flush=True)
    ...
    # Build disease features from SemMedDB
    disease_features = {}
    TOP_K = 8
    for dc in dcs_list:
        feats = semmeddb_feats.get(dc, {})  # cui -> count
        # Filter and rank
        ranked = sorted(feats.items(), key=lambda x: -x[1])
        names = []; seen = set()
        for cui, cnt in ranked:
            if cui in NOISE_GENERIC: continue
            if cui == dc: continue
            n_ = cp.get(cui, cui)
            nl = n_.lower().strip()
    ...
    print("\n샘플 SemMedDB features:")
    for sample in ["URTI", "Influenza", "PSVT", "Sarcoidosis", "Pneumonia", "Pericarditis"]:
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    np.save(f"pilot/results/v99_stage1_{n}.npy", score_matrix)
...
PICK: <number>"""
    ...
    print(f"\n  v99 (SemMedDB features + CoT) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v99 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
