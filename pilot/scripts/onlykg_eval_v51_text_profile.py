#!/usr/bin/env python3
"""v51: PubMed text-profile based scoring.

For each disease, count keyword occurrences in PubMed corpus:
- sudden vs gradual onset markers
- severe vs mild severity markers
- chronic vs acute markers

For patient features (intensity, suddenness from EVIDENCES), compute match
with disease's text profile.
"""
...
TEXT_PROFILES = "pilot/data/disease_text_profiles.json"
...
def main():
    ...
    ap.add_argument("--w_text", type=float, default=0.1)
    args = ap.parse_args()

    G = pickle.load(open(args.graph, "rb"))
    value_cuis = json.load(open(VALUE_CUIS))
    text_profiles = json.load(open(TEXT_PROFILES))
    ...
    # Normalize text profiles per dimension (z-score across diseases)
    profile_dims = ['sudden', 'gradual', 'severe', 'mild', 'chronic', 'fever_high']
    dim_stats = {}
    for dim in profile_dims:
        vals = [text_profiles.get(d, {}).get(dim, 0) for d in dcs_list]
        mean = sum(vals) / max(len(vals), 1)
        var = sum((v - mean)**2 for v in vals) / max(len(vals), 1)
        std = math.sqrt(var) or 1
        dim_stats[dim] = (mean, std)

    def disease_dim_score(d, dim):
        v = text_profiles.get(d, {}).get(dim, 0)
        mean, std = dim_stats[dim]
        return (v - mean) / std
    ...
    def parse_features(evs):
        ...
        return pcuis, compound_targets, intensity, suddenness, precision
    ...
    with open("data/ddxplus/release_test_patients.csv") as f:
        for row in csv.DictReader(f):
            ...
            pcuis, compound_targets, intensity, suddenness, precision = parse_features(evs)
            identity_diseases = pcuis & dcs_set

            s1_scores = {}; cov_scores = {}; prcov_scores = {}; comp_scores = {}; text_scores = {}
            for d in dcs_list:
                qp = d_q_idf.get(d, {})
                ...
                if suddenness is not None:
                    # high suddenness (≥7) → boost diseases with high "sudden" profile
                    s_factor = (suddenness - 5) / 5.0  # -1 to +1
                    t_score += s_factor * (disease_dim_score(d, 'sudden') - disease_dim_score(d, 'gradual'))
                if intensity is not None:
                    i_factor = (intensity - 5) / 5.0
                    t_score += i_factor * (disease_dim_score(d, 'severe') - disease_dim_score(d, 'mild'))
                text_scores[d] = t_score
            ...
            text_n = normalize_scores(text_scores)

            final = {d: args.w_s1*s1_n[d] + args.w_cov*cov_n[d] + args.w_prcov*prcov_n[d] + args.w_compound*comp_n[d] + args.w_text*text_n[d] for d in dcs_list}
    ...
    print(f"v51 text_profile w_text={args.w_text}: @1={100*c1/n:.2f}% @3={100*c3/n:.2f}% @5={100*c5/n:.2f}% @10={100*c10/n:.2f}% MRR={rr_sum/n:.4f}")
