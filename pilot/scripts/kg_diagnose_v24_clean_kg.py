#!/usr/bin/env python3
"""진단 v24: 노이즈 제거된 KG profile + LLM re-rank.

각 후보 질환의 KG 상위 증상을 prompt에 포함.
이전 v8(40.8%)은 노이즈로 실패. v24는 노이즈 자동 제거 후 시도.
"""
...
NOISE = {'C0150312','C0442743','C0039082','C0221423','C1457887','C0205390','C0442804','C3839861','C0332157','C1457868','C0445223','C1272751','C0015663','C0277814','C5202885','C0153933','C0585362',
         'C0021368','C0037088','C0460139','C0564405','C5203119','C0392756','C0277785'}  # noisy in many diseases
...
Candidate diagnoses with TYPICAL symptoms (from medical literature):
{candidates}

Match the patient's symptoms to the disease whose typical profile fits best.
Answer with ONLY the number (1-{n})."""


def main():
    print("="*80, flush=True)
    print("진단 v24: 노이즈 제거 KG + LLM re-rank", flush=True)
    print("="*80, flush=True)
    ...
    # 자동 노이즈 추가 (>= 30 질환에 공유)
    sym_disease = defaultdict(set)
    for (a, b), cnt in pc.items():
        if a in NOISE or b in NOISE: continue
        if a in dcs and b not in dcs: sym_disease[b].add(a)
        if b in dcs and a not in dcs: sym_disease[a].add(b)
    auto_noise = {s for s, ds_ in sym_disease.items() if len(ds_) >= 35 and cp.get(s, '') in
                  {'Inflammation','Signs and Symptoms','Pressure (finding)','Feeling relief','Intensity and Distress 5','Reduced','Functional disorder','Symptoms','Symptom'}}
    NOISE_ALL = NOISE | auto_noise
    print(f"  추가 노이즈: {len(auto_noise)}", flush=True)

    ds = defaultdict(dict); scuis = set()
    for (a, b), cnt in pc.items():
        if a in NOISE_ALL or b in NOISE_ALL: continue
    ...
    def disease_profile(dc, top=8):
        syms = sorted(ds.get(dc, {}).items(), key=lambda x: -x[1])[:top]
        return ", ".join(cp.get(s, s) for s, _ in syms)
    ...
    print("\n[4] 프롬프트 (KG profile 포함)...", flush=True)
    prompts = []
    for c in candidates:
        p = c["patient"]
        ...
        for i, dc in enumerate(c["topk"]):
            dname = cui2name.get(dc, dc)
            dprof = disease_profile(dc)
            cand_lines.append(f"{i+1}. {dname} (typical: {dprof})")
        cands = "\n".join(cand_lines)
    ...
    print(f"\n{'='*80}", flush=True)
    print(f"v24 GTPA@1 = {100*t1/nt:.1f}%", flush=True)
