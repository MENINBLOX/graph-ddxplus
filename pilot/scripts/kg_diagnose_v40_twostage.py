#!/usr/bin/env python3
"""진단 v40: 단일 프롬프트 내에서 hierarchical reasoning (body system narrowing).
...
def main():
    ...
    for c in candidates:
        ...
        prompt = f"""You are an expert clinician. Use systematic clinical reasoning.
...
CANDIDATES (each with typical features):
{cands}

Reasoning steps:
1. What ORGAN SYSTEM does the chief complaint and symptoms point to (e.g., cardiac, respiratory, ENT, GI, neuro, allergic, infectious)?
2. Among candidates from that system, which one's typical features BEST MATCH the patient's specific symptoms?
3. Use age, demographics, and symptom pattern to break ties.
