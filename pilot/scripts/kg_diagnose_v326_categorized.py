...
def main():
    ...
    print(f"[v326-categorized] Union: {n_medkg}/{len(disease_features)} have medkg, all have v87. Combined top-{TOP_K_FEATURES*2} per disease.")
    # v326: load categorized profiles (5 clinical categories)
    cat_path = Path("/mnt/medkg/kg/disease_categorized_by_cui.json")
    cat_by_cui = json.load(cat_path.open()) if cat_path.exists() else {}
    n_with_cat = sum(1 for d in diseases if cat_by_cui.get(name_to_cui[d]))
    print(f"[v326] Categorized profiles: {n_with_cat}/{len(diseases)} diseases")
    ...
    for c_idx, tied_dcs in small_ties:
        ...
        for i, dc in enumerate(tied_dcs):
            ...
            cat = cat_by_cui.get(dc, {})
            if cat:
                cat_lines = []
                for c_label in ["EXPOSURE","SYSTEMIC","RESPIRATORY","PAIN_PROFILE","COMORBIDITY"]:
                    items = cat.get(c_label, [])
                    if items:
                        cat_lines.append(f"      {c_label}: {'; '.join(items[:3])}")
                if cat_lines:
                    block += "\n   Clinical context:\n" + "\n".join(cat_lines)
            lines.append(block)
        disease_list = "\n".join(lines)
