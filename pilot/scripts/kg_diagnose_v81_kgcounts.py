#!/usr/bin/env python3
"""진단 v81: v76 + KG feature counts in prompt.

각 KG feature에 PubMed 빈도 (count) 포함:
"Typical features: fever (n=125), cough (n=98), dyspnea (n=87)..."
LLM이 feature 가중치를 인지 가능.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v81: KG features WITH COUNTS in prompt", flush=True)
    print("="*80, flush=True)
    ...
    for dc in dcs_list:
        ...
        for cui, cnt in top_cuis:
            ...
            seen_lower.add(nl)
            names_with_count.append(f"{n} (n={cnt})")
            if len(names_with_count) >= TOP_K_FEATURES: break
        disease_features[dc] = ", ".join(names_with_count) if names_with_count else "—"
...
Typical features (PubMed co-occurrence counts): {kg_feats}
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    np.save("pilot/results/v81_score_matrix.npy", score_matrix)
    ...
    print(f"\n  v81 (KG features + counts) @1={100*t1/n:.1f}% @3={100*t3/n:.1f}% @5={100*t5/n:.1f}% @10={100*t10/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v81 GTPA@1 = {100*t1/n:.1f}% (SUBSET={n})", flush=True)
