#!/usr/bin/env python3
"""RareBench 환자에서 추출한 질환 이름으로 PubMed KG 구축.

원칙: RareBench에서 질환 이름만 사용. 환자의 HPO 표현형은 KG 구축에 미사용.
"""
...
KG_CACHE = RESULTS_DIR / "kg_rarebench_cache.json"
...
MAX_ABSTRACTS = 200  # 441 질환이라 작게


def main():
    print("="*80, flush=True)
    print("RareBench 질환 PubMed KG 구축", flush=True)
    print("="*80, flush=True)

    # RareBench 환자에서 질환 추출
    diseases_codes = set()
    ...
    for code in diseases_codes:
        if code in dm:
            ...
            if cui not in cui_to_disease:
                cui_to_disease[cui] = {"name": dm[code]["umls_name"], "code": code}

    diseases = {info["name"]: {"cui": cui, "umls_name": info["name"]} for cui, info in cui_to_disease.items()}
    print(f"질환: {len(diseases)} (RareBench {len(diseases_codes)}개 → UMLS {len(diseases)}개 고유 CUI)", flush=True)
        ...
        for pmid, ab in rows:
            ...
            if cuis:
                ...
                tasks.append({"prompt": PROMPT.format(abstract=ab[:3000],
                    ...
                    "dc": dc})
        if (idx+1) % 50 == 0:
            print(f"  [{idx+1}/{len(diseases)}] {len(tasks):,} 누적 프롬프트", flush=True)
    conn.close()
    ...
    with open(KG_CACHE, "w") as f: json.dump(cache_data, f)
