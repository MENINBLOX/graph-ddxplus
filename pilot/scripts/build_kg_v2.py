#!/usr/bin/env python3
"""KG 구축 V2: 텍스트 매칭 + 질환중심 LLM.

파이프라인:
  1. Aho-Corasick으로 UMLS DISO CUI 영문 이름 사전 구축
  2. 질환 CUI → PubMed SQLite 검색 (MeSH + 키워드, 다중 후보)
  3. 초록 텍스트에서 Aho-Corasick으로 DISO CUI 추출
  4. 질환중심 프롬프트: "이 초록에서 [질환]과 관련된 CUI는?" → LLM 1회/초록
  5. 통계 집계 (MC≥N) + CUI 1-level 전파 → KG
  6. DDXPlus 벤치마크 + MC sweep
"""
...
ALLOWED_STYS = {"T047", "T184", "T191", "T046", "T048", "T037", "T019", "T020", "T190", "T049",
                "T033", "T031", "T040"}  # +Finding, Body Substance, Organism Function
...
CKPT_FILE = DATA_DIR / "kg_v2c_checkpoint.json"
RESULTS_FILE = RESULTS_DIR / "kg_v2c_results.json"
...
def search_abstracts(conn, disease_cui, disease_name, umls_name, cui_to_mesh, limit):
    c = conn.cursor()
    rows = []
    seen_pmids = set()
    ...
    # 2) 키워드 검색 보충 (DDXPlus 이름 + UMLS 이름 + "/" 분리)
    if len(rows) < limit:
        candidates = set()
        for raw in [umls_name, disease_name]:
            kw = re.sub(r'\(.*?\)', '', raw)
        ...
        for kw in candidates:
            ...
            exclude = ",".join(f"'{p}'" for p in seen_pmids) if seen_pmids else "''"
            c.execute(f"""
                SELECT pmid, abstract FROM abstracts
...
def main():
    print("=" * 80)
    print("KG 구축 V2: 텍스트 매칭 + 질환중심 LLM")
    print("=" * 80)
    ...
    if ckpt["phase"] == "init":
        ...
        for idx, (disease_name, dinfo) in enumerate(sorted(disease_cuis.items())):
            ...
            if dc in ckpt["docs"]:
                ...
                print(f"  [{idx+1}/49] {disease_name}: 이미 {len(ckpt['docs'][dc])}편 (건너뜀)")
                continue

            rows = search_abstracts(conn, dc, disease_name, dn, cui_to_mesh, MAX_ABSTRACTS_PER_DISEASE)
            ...
            ckpt["docs"][dc] = disease_docs
            total_docs += len(disease_docs)
            avg_cuis = sum(len(d["cuis"]) for d in disease_docs) / max(len(disease_docs), 1)
            print(f"  [{idx+1}/49] {disease_name}: "
                  f"검색={len(rows)}편, CUI≥1={len(disease_docs)}편, 평균CUI={avg_cuis:.0f}")
            save_ckpt()
        ...
        total_docs = sum(len(docs) for docs in ckpt["docs"].values())
        print(f"\n[2/6] CUI 추출 완료 (체크포인트): {total_docs}편")

    # [3] LLM 분류 준비
    print(f"\n[3/6] LLM 분류 준비...")

    # (disease_cui, pmid)를 키로 사용
    all_tasks = []
    ...
    # 쌍 카운트: (disease_cui, related_cui) → count
    pair_counts = Counter()
    ...
    print(f"  고유 질환-CUI 쌍: {len(pair_counts):,}")
    ...
    def build_kg(mc):
        ...
        return kg_raw, expanded

    best_f1 = 0
    best_mc = 1
    best_result = None

    print(f"\n  MC sweep (CUI 1-level 전파):")
    for mc in [1, 2, 3, 5, 10]:
        kg_raw, kg_exp = build_kg(mc)
        ev = evaluate(kg_exp, gold_pairs, parent_map)
        marker = " ★" if ev["F1"] > best_f1 else ""
        print(f"    MC={mc:>2d}: raw={len(kg_raw):>6,} exp={len(kg_exp):>6,} "
    ...
    for mc in [1, 2, 3]:
        ...
        print(f"    MC={mc:>2d}: raw={len(kg_raw):>6,} "
              f"P={ev['P']:.3f} R={ev['R']:.3f} F1={ev['F1']:.3f} "
              f"match={ev['matched']}/{ev['gold']}")

    # [6] 저장
    print(f"\n[6/6] 결과 저장...")
    kg_raw, kg_exp = build_kg(best_mc)
