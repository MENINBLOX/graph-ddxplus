...
def main():
    ...
    print(f"[v304-hybrid-gap-ddxplus] Union: {n_medkg}/{len(disease_features)} have medkg, all have v87. Combined top-{TOP_K_FEATURES*2} per disease.")
    ...
    # v303: HYBRID stage1 = 0.7 * v79 (PubMed cooccur) + 0.3 * v302 (dual KG features)
    # Both normalized to [0, 100] per row before blending.
    v79 = np.load("pilot/results/v79_stage1.npy")
    v302 = np.load("pilot/results/v302_stage1_5000.npy")
    def _norm(m):
        rng = m.max(axis=1, keepdims=True) - m.min(axis=1, keepdims=True) + 1e-6
        return (m - m.min(axis=1, keepdims=True)) / rng * 100
    score_matrix = 0.7 * _norm(v79) + 0.3 * _norm(v302)
    n = score_matrix.shape[0]
    print(f"v303 hybrid stage1: {n} patients × {len(dcs_list)} candidates (0.7*v79 + 0.3*v302)", flush=True)
    ...
    # v304: gap-based tied detection (hybrid scores are continuous, no exact ties)
    # Treat top-K candidates within score gap of TIE_GAP from max as "tied"
    TIE_GAP = 5.0
    MAX_TIED = 10
    tied_patients = []
    for c_idx in range(n):
        ...
        tied_idxs = [i for i in ranked if (max_score - scores[i]) <= TIE_GAP][:MAX_TIED]
        if len(tied_idxs) >= 2:
            tied_dcs = [dcs_list[i] for i in tied_idxs]
            tied_patients.append((c_idx, tied_dcs))
    small_ties = tied_patients
    print(f"  Gap-tied (≤{TIE_GAP}): {len(tied_patients)} ({100*len(tied_patients)/n:.1f}%), capped at {MAX_TIED}", flush=True)
...
PICK: <number>"""
    ...
    print(f"\n  v304 (hybrid-gap+CoT tie-break) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v304 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
