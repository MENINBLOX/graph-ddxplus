#!/usr/bin/env python3
"""진단 v68: v59→top10→discriminator features (UNIQUE per candidate) re-score.
...
def main():
    ...
    for p in patients:
        ...
        # v59: use ALL 49 disease candidates (no KG top-K filter)
        topk = list(dcs)
...
How likely is this hypothesis given the patient's presentation? Reply with ONLY a percentage 0-100 (0=impossible, 100=certain)."""
    ...
    # Stage 1: Rank by v59 LLM score, take top-10
    print("\n[6] Stage 1: v59 score → top-10", flush=True)
    stage1_top10 = []
    t1_v59 = 0
    in_top10_v59 = 0
    for c_idx, c in enumerate(candidates):
    ...
    # Stage 2: Re-score top-10 with v54-style prompt (different framing)
    # v68: Compute UNIQUE features for each candidate within top-10
    sym_df_clean = Counter()
    for d, syms in ds.items():
        for s in syms: sym_df_clean[s] += 1
    N_d_clean = len(dcs)
    idf_clean = {s: math.log(N_d_clean / df) for s, df in sym_df_clean.items()}

    def get_top_syms(dc, n=15):
        syms = ds.get(dc, {})
        scored = [(s, c * idf_clean.get(s, 0)) for s, c in syms.items() if sym_df_clean.get(s, 99) <= 30]
        scored.sort(key=lambda x: -x[1])
        return set(s for s, _ in scored[:n])

    print("\n[7] Stage 2: discriminating features for each top-10 candidate", flush=True)
    convs2 = []
    pair_meta2 = []
    for c_idx, c in enumerate(candidates):
        p = c["patient"]
        ...
        chief = re.sub(r"Do you |Have you |Are you ", "", chief).rstrip("?").strip()
        age_sex = f"{p['age']}yo {'Male' if p['sex']=='M' else 'Female'}"
        # All top syms across top-10 candidates
        all_top_syms = {dc: get_top_syms(dc) for dc in stage1_top10[c_idx]}
        for d_idx, dc in enumerate(stage1_top10[c_idx]):
            # Unique features = this candidate's top - others' top
            this_top = all_top_syms[dc]
            others_top = set()
            for other_dc in stage1_top10[c_idx]:
                if other_dc != dc: others_top.update(all_top_syms[other_dc])
            unique = this_top - others_top
            unique_names = [cp.get(s, "") for s in unique][:5]
            unique_names = [n for n in unique_names if n]
            unique_str = ", ".join(unique_names) if unique_names else "(none distinctive)"
            name = disease_full_name(cui2name.get(dc, dc))
            prompt = f"""Patient: {age_sex}
Chief complaint: {chief}
{profile}
...
Diagnosis hypothesis: {name}
Features UNIQUE to this diagnosis (not in other top candidates): {unique_str}

How likely is this hypothesis? Pay special attention to whether the patient has the unique features. Reply with ONLY a percentage 0-100."""
    ...
    print(f"  v59 stage1 @1={100*t1_v59/nt:.1f}%, top10={100*in_top10_v59/nt:.1f}%", flush=True)
    print(f"  v66 stage2 @1={100*t1/nt:.1f}% @3={100*t3/nt:.1f}% @5={100*t5/nt:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v66 GTPA@1 = {100*t1/nt:.1f}% (SUBSET={nt})", flush=True)
