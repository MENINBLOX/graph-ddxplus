...
def main():
    ...
    print(f"[v111-union-rarebench] {n_medkg}/{len(disease_features)} have medkg, all have v87. Combined top-{TOP_K*2}.", flush=True)
    ...
    # === v112: build medkg HP-based disease→phenotype set for backup Bayesian ===
    medkg_hp_per_disease = {}  # cui → set of HP IDs
    try:
        from collections import defaultdict as _dd
        kg_path = Path("/home/max/Graph-DDXPlus/data/medkg/kg/kg_merged.jsonl")
        if kg_path.exists():
            with kg_path.open() as f:
                for line in f:
                    ...
                    cui = e.get("umls_cui")
                    hpo = e.get("hpo_id")
                    if cui and hpo and hpo.startswith("HP:"):
                        medkg_hp_per_disease.setdefault(cui, set()).add(hpo)
        print(f"  medkg HP-per-disease: {len(medkg_hp_per_disease)} diseases with HP IDs", flush=True)
    except Exception as ex:
        print(f"  medkg HP load failed: {ex}", flush=True)

    # Bayesian top-10 filter — UNION of v87 CUI Bayesian + medkg HP overlap count
    print("\nBayesian top-10 filter (v87 CUI + medkg HP-direct)...", flush=True)
    candidates = []; baseline = 0
    for p in valid_patients:
        patient_hps = set(h for h in p["phen_cuis"] if h.startswith("HP:"))
        sc = {}
        for dc in dcs:
            s = ds.get(dc, {})
            v87_score = -1e6
            if s:
                ...
                v87_score = sum(math.log((s[x]+0.1)/tw+1e-10) if x in s else math.log(0.1/tw+1e-10) for x in p["phen_cuis"])
            # medkg HP overlap (count of patient HPs in disease's medkg HP set)
            hp_set = medkg_hp_per_disease.get(dc, set())
            hp_overlap = len(patient_hps & hp_set)
            # Combine: v87 score + scaled HP overlap (so HP signal helps but doesn't overwhelm)
            sc[dc] = v87_score + hp_overlap * 2.0  # weight HP overlap
    ...
    print(f"  v112-Bayesian @1={100*baseline/n:.1f}%, top10={100*in_top10/n:.1f}%", flush=True)
