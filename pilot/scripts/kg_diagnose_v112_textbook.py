...
def main():
    ...
    # === v112 TEXTBOOK-only hybrid: textbook IE where available, v87 PubMed-only elsewhere ===
    # Textbook KG = StatPearls + GeneReviews + MedlinePlus + Wikipedia (Orphanet excluded)
    medkg_cui_path = Path("/mnt/medkg/kg/disease_features_textbook_by_cui.json")
    medkg_path = Path("/mnt/medkg/kg/disease_features_textbook.json")
    TOP_K_FEATURES = 8
    ...
    # Overlay medkg features where available
    disease_features = dict(v87_features)
    n_medkg = 0
    for dn in diseases:
        cui = name_to_cui[dn]
        feats = medkg_cui.get(cui) or medkg.get(dn, [])
        if feats:
            names = [f["phenotype"] for f in feats[:TOP_K_FEATURES]]
            if names:
                disease_features[cui] = ", ".join(names)
                n_medkg += 1
    print(f"[v112-textbook-ddxplus] Hybrid: {n_medkg}/{len(disease_features)} diseases use textbook IE, rest use v87 PubMed-only")
