...
def main():
    ...
    # === v322: load disease prior P(D) from train data ===
    prior_path = Path("/mnt/medkg/kg/disease_prior_by_cui.json")
    disease_prior = json.load(prior_path.open()) if prior_path.exists() else {}
    print(f"[v322] Loaded {len(disease_prior)} disease priors")
    ...
    for chunk_start in range(0, len(tb_convs), CHUNK):
        ...
        for i, out in enumerate(outs):
            ...
            if picks:
                ...
                # v322: combine LLM vote count with disease prior
                vote_counts = _C(picks)
                # Score = vote_count + alpha * log(prior)
                ALPHA = 1.5
                import math
                best_pick = None; best_score = -1e9
                for p, vc in vote_counts.items():
                    dc_at_p = tied_dcs[p]
                    prior = disease_prior.get(dc_at_p, 0.001)
                    score = vc + ALPHA * math.log(prior + 1e-6)
                    if score > best_score:
