#!/usr/bin/env python3
"""진단 v95: v87 (CoT tie-break) + present/absent feature highlighting.

For each tied candidate disease, list:
- Features PRESENT in patient (intersect with KG features by text-match)
- Features ABSENT (typical KG features not in patient text)

This gives LLM explicit positive/negative signal beyond raw KG list.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v95: CoT tie-break with present/absent feature analysis", flush=True)
    print("="*80, flush=True)
    ...
    # KG features as list of strings (for matching)
    disease_features_list = {}  # dc -> list of (name, lowercase_name)
    TOP_K = 12  # bigger to catch more matches
    for dc in dcs_list:
        feats = ds.get(dc, {})
        ...
        for cui, cnt in top_cuis:
            ...
            if len(nl) < 4 or len(nl) > 50: continue
            seen.add(nl); names.append((n_, nl))
            if len(names) >= TOP_K: break
        disease_features_list[dc] = names

    def patient_profile_text(evidences):
        """Return both structured profile AND free-text version for matching."""
        ...
        struct = "\n".join(out)
        flat_text = struct.lower()
        return struct, flat_text
    ...
    patient_data = []
    for c in candidates:
        struct, flat = patient_profile_text(c["evidences"])
        ie = c.get("initial", "")
        ...
        patient_data.append({"profile": struct, "flat": flat, "chief": chief, "age_sex": age_sex})
    ...
    print(f"  Tied: {len(tied_patients)}, small≤10: {len(small_ties)}", flush=True)
    ...
    chosen_dc = {}
    print(f"\n[Stage 2] CoT with present/absent features...", flush=True)
    tb_convs = []; tb_meta = []
    for c_idx, tied_dcs in small_ties:
        ...
        for i, dc in enumerate(tied_dcs):
            ...
            feats = disease_features_list.get(dc, [])
            present = []; absent = []
            for orig, lower in feats:
                # Check if any word from the feature is in patient text
                # Simple substring check
                if lower in flat:
                    present.append(orig)
                else:
                    # Try keyword match (first significant word)
                    key = lower.split()[0] if ' ' in lower else lower
                    if len(key) >= 4 and key in flat:
                        present.append(orig)
                    else:
                        absent.append(orig)
            present_str = ", ".join(present[:6]) if present else "none observed"
            absent_str = ", ".join(absent[:6]) if absent else "all observed"
            lines.append(f"({i+1}) {name}\n      PRESENT: {present_str}\n      ABSENT/UNREPORTED: {absent_str}")
        disease_list = "\n".join(lines)
...
The following candidates are equally likely so far. For each, the typical features have been split into PRESENT and ABSENT/UNREPORTED in this patient. Pick the SINGLE most likely based on which has the best match (more PRESENT, fewer ABSENT critical features).
...
PICK: <number>"""
    ...
    print(f"\n  v95 (present/absent CoT) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v95 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
