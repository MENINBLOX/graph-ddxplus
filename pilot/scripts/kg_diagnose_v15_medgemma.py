#!/usr/bin/env python3
"""진단 v15: medgemma-1.5-4b re-ranking: 구조화 프로필 + 전체 antecedent + 감별진단 프롬프트.
...
def main():
    print("=" * 80, flush=True)
    print("진단 v15: medgemma-1.5-4b re-ranking: 구조화 + 전체 antecedent + 감별 프롬프트", flush=True)
    print("=" * 80, flush=True)
    ...
    llm = LLM(model="google/medgemma-1.5-4b-it", dtype="bfloat16", max_model_len=4096,
