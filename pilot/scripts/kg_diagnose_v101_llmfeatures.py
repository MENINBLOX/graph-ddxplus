#!/usr/bin/env python3
"""진단 v101: v87 framework with LLM-generated KG features.

Use gemma-4-E4B's pretrained medical knowledge to generate textbook-quality
disease features (instead of PubMed cooccurrence).

URTI features:
  PubMed v87: Urticaria, Rhinitis, Fever, Acute pharyngitis, Asthma, Angioedema (noisy)
  LLM v101:   Rhinorrhea, Nasal congestion, Sore throat, Cough, Sneezing, Mild fever (clean)
"""
...
LLM_FEATURES = Path("pilot/results/llm_disease_features.json")
...
def main():
    print("="*80, flush=True)
    print("진단 v101: v87 + LLM-generated KG features", flush=True)
    print("="*80, flush=True)

    with open("data/ddxplus/disease_icd10_cui_mapping.json") as f: icd_map = json.load(f)
    with open("data/ddxplus/release_conditions_en.json") as f: cond = json.load(f)
    with open("data/ddxplus/release_evidences.json") as f: ev_fr = json.load(f)
    with open(LLM_FEATURES) as f: llm_feats = json.load(f)
    ...
    # Build disease features from LLM-generated
    disease_features = {}
    for dc in dcs_list:
        feats = llm_feats.get(dc, [])
        disease_features[dc] = ", ".join(feats[:8]) if feats else "—"

    print("\n샘플 LLM features:")
    for sample in ["URTI", "Influenza", "PSVT", "Pneumonia", "Sarcoidosis", "Pericarditis"]:
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    np.save(f"pilot/results/v101_stage1_{n}.npy", score_matrix)
...
PICK: <number>"""
    ...
    print(f"\n  v101 (LLM features + CoT) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v101 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
