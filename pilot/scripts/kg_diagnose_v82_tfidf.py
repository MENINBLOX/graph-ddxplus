#!/usr/bin/env python3
"""진단 v82: v76 + TF-IDF discriminative KG features.

v76의 KG features는 disease별 단순 top-N count → 일반증상(Fever, Pain) 다수.
v82는 TF-IDF 가중치로 차별성 높은 features만 선별:

  TF(d, f) = count(d, f) / sum_f' count(d, f')         # disease 내 비율
  IDF(f)   = log(N_diseases / num_diseases_with(f))    # cross-disease 희소성
  score    = TF × IDF                                   # 차별성

이로써 "Pain (48/49)", "Signs and Symptoms (45/49)" 같은 무차별 features는 자동 제외.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v82: TF-IDF discriminative KG features", flush=True)
    print("="*80, flush=True)
    ...
    # Build per-disease feature counts
    ds = defaultdict(dict)
    ...
    # IDF: how many diseases mention this feature CUI
    ...
    print(f"\n[1] TF-IDF KG features 구축 (N={N_DISEASES} diseases)", flush=True)

    # Compute TF-IDF for each (disease, feature)
    disease_features = {}
    TOP_K_FEATURES = 8
    for dc in dcs_list:
        feats = ds.get(dc, {})
        ...
        for cui, cnt, sc in scored:
            ...
            # Skip identical to disease name
    ...
    # Print samples for inspection (esp. problem cases from analysis)
    print("\n샘플 (v76 vs v82):")
    for sample_name in ["URTI", "Influenza", "PSVT", "Pneumonia", "Sarcoidosis", "Stable angina", "Unstable angina"]:
        sample_dc = None
        for dn, info in diseases.items():
            if dn == sample_name:
                sample_dc = info["cui"]; break
        if sample_dc:
            print(f"  {sample_name}: {disease_features[sample_dc]}", flush=True)
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    print(f"\n  v82 (TF-IDF KG features) @1={100*t1/n:.1f}% @3={100*t3/n:.1f}% @5={100*t5/n:.1f}% @10={100*t10/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v82 GTPA@1 = {100*t1/n:.1f}% (SUBSET={n})", flush=True)
