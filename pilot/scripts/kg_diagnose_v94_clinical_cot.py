#!/usr/bin/env python3
"""진단 v94: clinical-structured CoT for tied cases (v87 변형).

v87 prompt: brief evaluation + PICK
v94 prompt: explicit clinical reasoning - "key findings", "supporting/refuting evidence per candidate", "likelihood"

가설: 더 구조화된 임상 추론이 LLM의 pick 정확도 향상.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v94: clinical-structured CoT tie-break", flush=True)
    print("="*80, flush=True)
    ...
    chosen_dc = {}
    print(f"\n[Stage 2] Clinical CoT tie-break...", flush=True)
    tb_convs = []; tb_meta = []
    for c_idx, tied_dcs in small_ties:
        ...
        for i, dc in enumerate(tied_dcs):
            ...
            lines.append(f"({i+1}) {name} | textbook features: {kg_feats}")
        disease_list = "\n".join(lines)

        prompt = f"""You are a clinician evaluating a {pd['age_sex']} patient.

PRESENTATION:
Chief complaint: {pd['chief']}
{pd['profile']}

DIFFERENTIAL CONSIDERATIONS (currently equal weight - need to differentiate):
{disease_list}

CLINICAL REASONING TASK:
For EACH numbered candidate, list:
- Supporting features observed in this patient
- Features expected but NOT observed (or contradicting features)

Then, based on which candidate has the strongest support and weakest contradictions, choose the SINGLE most likely diagnosis.

Format:
(1) Supports: ... | Missing/contradicts: ...
(2) Supports: ... | Missing/contradicts: ...
...
DIAGNOSIS: <number>"""
    ...
    for chunk_start in range(0, len(tb_convs), CHUNK):
        ...
        for i, out in enumerate(outs):
            ...
            m = re.search(r"DIAGNOSIS\s*:\s*\(?(\d+)\)?", text, re.IGNORECASE)
            if not m: m = re.search(r"PICK\s*:\s*\(?(\d+)\)?", text, re.IGNORECASE)
    ...
    print(f"\n  v94 (clinical CoT) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v94 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
