#!/usr/bin/env python3
"""진단 v102: v87 framework with patient-reportable features only.

DDXPlus evidence는 모두 자가인지 가능한 증상이므로, KG features 중 의학검사로만
알 수 있는 것 (Atrial Fibrillation on ECG, Hilar lymphadenopathy on X-ray,
Granuloma on biopsy, Crackles on auscultation 등)을 제거하면 매칭 신호가 정확해진다.

v87 PubMed top-8 cooccurrence + LLM-classified patient-reportable filter.
"""
from __future__ import annotations
import ast, csv, json, os, re, time, sys
...
PR_CLASSIFICATION = Path("pilot/results/feature_patient_reportable.json")
...
def main():
    print("="*80, flush=True)
    print("진단 v102: v87 + patient-reportable features only", flush=True)
    print("="*80, flush=True)
    ...
    with open(PR_CLASSIFICATION) as f: pr = json.load(f)
    ...
    # Build features filtered to patient-reportable only
    disease_features = {}
    TOP_K = 8
    for dc in dcs_list:
        feats = ds.get(dc, {})
        # Sort by count, filter to Y (patient-reportable)
        ranked = sorted(feats.items(), key=lambda x: -x[1])
        names = []; seen = set()
        for cui, cnt in ranked:
            if pr.get(cui, 'N') != 'Y':
                continue  # skip clinical-only
            if cui == dc: continue
            n_ = cp.get(cui, cui)
            nl = n_.lower().strip()
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    np.save(f"pilot/results/v102_stage1_{n}.npy", score_matrix)
...
PICK: <number>"""
    ...
    print(f"\n  v102 (patient-reportable filter + CoT) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v102 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
