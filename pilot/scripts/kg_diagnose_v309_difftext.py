...
def main():
    ...
    print(f"[v309-difftext] Union: {n_medkg}/{len(disease_features)} have medkg, all have v87. Combined top-{TOP_K_FEATURES*2} per disease.")

    # v309: load per-disease differential/diagnosis raw text (max 600 chars per disease)
    diff_text_path = Path("/mnt/medkg/kg/disease_diff_text.json")
    diff_text = json.load(diff_text_path.open()) if diff_text_path.exists() else {}
    # truncate per-disease text
    diff_text_short = {}
    for cui, txt in diff_text.items():
        # take first 600 chars to fit prompt budget
        diff_text_short[cui] = (txt or "")[:600].strip()
    n_with_diff = sum(1 for d in diseases if diff_text_short.get(name_to_cui[d]))
    print(f"[v309] {n_with_diff}/{len(diseases)} diseases have differential/diagnosis text")
    ...
    print(f"\n[Stage 2] CoT tie-break with discrimination text...", flush=True)
    tb_convs = []; tb_meta = []
    for c_idx, tied_dcs in small_ties:
        ...
        for i, dc in enumerate(tied_dcs):
            ...
            dt = diff_text_short.get(dc, "")
            block = f"({i+1}) {name} — typical features: {kg_feats}"
            if dt:
                block += f"\n    Diagnosis context: {dt}"
            lines.append(block)
        disease_list = "\n".join(lines)
