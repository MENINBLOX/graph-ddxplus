#!/usr/bin/env python3
"""진단 v53: v50 LLM score + Bayesian likelihood 가중 결합.
...
def main():
    ...
    for p in patients:
        ...
        # Save Bayesian scores for top-K (normalized to [0, 1] within top-K)
        topk_bay = [sc[dc] for dc in topk]
        max_b = max(topk_bay)
        min_b = min(topk_bay)
        rng = (max_b - min_b) or 1
        topk_bay_norm = [(b - min_b) / rng for b in topk_bay]
        candidates.append({"patient": p, "true_dc": tdc, "topk": topk, "bay_norm": topk_bay_norm})
...
How likely is this hypothesis given the patient's presentation? Reply with ONLY a single digit 0-9 (0=very unlikely, 9=very likely)."""
    ...
    # Test multiple weights for combining LLM score + Bayesian normalized
    nt = len(candidates)
    print(f"\n  Baseline @1={100*baseline/nt:.1f}%", flush=True)

    # v50 only (alpha=1.0)
    print("\n[7] Combination 평가:", flush=True)
    for alpha in [1.0, 0.9, 0.8, 0.7, 0.5, 0.3, 0.0]:
        t1 = t3 = t5 = 0
        for c_idx, c in enumerate(candidates):
            scored = []
            for d_idx in range(len(c["topk"])):
                llm_score = all_scores.get((c_idx, d_idx), 0) / 9.0  # normalize to 0-1
                bay_score = c["bay_norm"][d_idx]
                combined = alpha * llm_score + (1 - alpha) * bay_score
        ...
        print(f"  α={alpha:>3.1f} (LLM={alpha}, Bay={1-alpha:.1f}): @1={100*t1/nt:.1f}% @3={100*t3/nt:.1f}% @5={100*t5/nt:.1f}%", flush=True)

    print(f"\n{'='*80}", flush=True)
    print(f"v53 best (depends on alpha)", flush=True)
    print(f"v53 GTPA@1 = -- (see above)", flush=True)
