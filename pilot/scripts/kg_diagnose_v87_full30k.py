...
Stage 1: v76-style 49 candidates × 30000 patients (1.47M LLM calls, ~75 min)
Stage 2: CoT tie-break for ties (~5 min)
...
def main():
    print("="*80, flush=True)
    print("v87 full 30K confirmation", flush=True)
    print("="*80, flush=True)
    ...
    # ===== Stage 1: 49-cand scoring =====
    print(f"\n[Stage 1] 49-cand scoring on {n} patients...", flush=True)
    convs = []; pair_meta = []
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    for chunk_start in range(0, len(convs), CHUNK):
        ...
        if chunk_start % 50000 == 0 or chunk_start + CHUNK >= len(convs):
    ...
    np.save(f"pilot/results/v87full_stage1_{n}.npy", score_matrix)
...
PICK: <number>"""
    ...
    np.save(f"pilot/results/v87full_final_{n}.npy", final_score)
    ...
    print(f"\n  v87 full ({n} patients) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
