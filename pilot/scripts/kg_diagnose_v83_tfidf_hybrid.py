#!/usr/bin/env python3
"""진단 v83: v82 (TF-IDF features) + v79 2-stage hybrid.

Stage 1: 49 candidates × all patients with TF-IDF KG features → top-10
Stage 2: rescore top-10 with TF-IDF KG features + "top-10 most likely" framing
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v83: TF-IDF KG features + 2-stage hybrid", flush=True)
    print("="*80, flush=True)
    ...
    # ===== Stage 1: TF-IDF KG features × 49 candidates =====
    print(f"\n[Stage 1] TF-IDF KG features 49-candidate scoring...", flush=True)

    # Pre-build patient-side data
    patient_data = []
    for c in candidates:
        p = c["patient"]
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    np.save("pilot/results/v83_stage1.npy", score_matrix)
    ...
    # ===== Stage 2: rescore top-10 =====
    print(f"\n[Stage 2] top-10 rescoring...", flush=True)
    TOP_K = 10
    top_k_per_patient = []
    for c_idx in range(n):
        ranked = np.argsort(-score_matrix[c_idx])[:TOP_K]
        top_k_per_patient.append([dcs_list[i] for i in ranked])

    convs2 = []; pair_meta2 = []
    for c_idx, c in enumerate(candidates):
        pd = patient_data[c_idx]
        for d_idx_in_top, dc in enumerate(top_k_per_patient[c_idx]):
            name = disease_full_name(cui2name.get(dc, dc))
            kg_feats = disease_features[dc]
            prompt = f"""Patient: {pd['age_sex']}
Chief complaint: {pd['chief']}
...
Among ALL possible diagnoses, this is one of the top-10 most likely. How likely is THIS specific hypothesis given the patient? Reply with ONLY a percentage 0-100."""
            ...
            pair_meta2.append((c_idx, d_idx_in_top))

    print(f"  Stage 2 LLM calls: {len(convs2)}", flush=True)
    final_score = np.full((n, len(dcs_list)), -1.0, dtype=np.float32)
    t1 = time.time()
    for chunk_start in range(0, len(convs2), CHUNK):
        chunk = convs2[chunk_start:chunk_start+CHUNK]
        outs = llm.chat(chunk, sampling)
        for i, out in enumerate(outs):
            c_idx, d_idx_in_top = pair_meta2[chunk_start + i]
            dc = top_k_per_patient[c_idx][d_idx_in_top]
            d_idx = dcs_list.index(dc)
            text = out.outputs[0].text.strip()
            m = re.search(r"(\d+)", text)
            score = int(m.group(1)) if m else 0
            score = min(score, 100)
            final_score[c_idx, d_idx] = score
        print(f"  Stage 2: {chunk_start+len(chunk)}/{len(convs2)} ({time.time()-t1:.0f}초)", flush=True)
    print(f"  Stage 2 완료: {time.time()-t1:.0f}초", flush=True)
    np.save("pilot/results/v83_final.npy", final_score)
    ...
    print(f"\n  v83 (TF-IDF + 2-stage) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v83 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
