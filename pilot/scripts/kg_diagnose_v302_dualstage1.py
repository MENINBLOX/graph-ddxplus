#!/usr/bin/env python3
"""진단 v76: KG features in prompt + per-candidate scoring.

각 후보 disease의 top KG features (PubMed 빈도순)를 prompt에 포함 → LLM이
해당 질환의 typical signature 인지 후 환자와 비교.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v76: KG features in prompt", flush=True)
    print("="*80, flush=True)
    ...
    # v302: Build disease features from UNION of v87 PubMed cooccurrence + dual KG (textbook + PubMed IE)
    ds = defaultdict(dict)
    ...
    n_medkg = 0
    for dc in dcs_list:
        # v87 PubMed cooccurrence top features
        feats = ds.get(dc, {})
        ...
        for cui, cnt in top_cuis:
            ...
            seen_lower.add(nl); v87_names.append(n)
            if len(v87_names) >= TOP_K_FEATURES: break
        # dual KG (textbook + PubMed IE) top features
        dn = cui2name.get(dc, dc)
        feats_medkg = medkg_cui.get(dc) or medkg.get(dn, [])
        medkg_terms = [f["phenotype"] for f in feats_medkg[:TOP_K_FEATURES]] if feats_medkg else []
        if medkg_terms: n_medkg += 1
        # UNION (deduped, dual KG first for novelty bias)
        combined = []
        seen2 = set()
        for t in medkg_terms + v87_names:
            tl = t.lower().strip()
            if not tl or tl in seen2: continue
            seen2.add(tl); combined.append(t)
            if len(combined) >= TOP_K_FEATURES * 2: break
        disease_features[dc] = ", ".join(combined) if combined else "—"
    print(f"  Stage1 features (v87 ∪ dual KG): {n_medkg}/{len(disease_features)} have medkg, all have v87. Top-{TOP_K_FEATURES*2}.", flush=True)

    # Sample print
    print(f"\n샘플 KG features:", flush=True)
    sample_dc = dcs_list[0]
    print(f"  {cui2name.get(sample_dc, sample_dc)}: {disease_features[sample_dc]}", flush=True)
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    np.save("pilot/results/v302_stage1_5000.npy", score_matrix)
    ...
    print(f"\n  v76 (KG features) @1={100*t1/n:.1f}% @3={100*t3/n:.1f}% @5={100*t5/n:.1f}% @10={100*t10/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v76 GTPA@1 = {100*t1/n:.1f}% (SUBSET={n})", flush=True)
