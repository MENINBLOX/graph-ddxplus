#!/usr/bin/env python3
"""v41: Score normalization per channel before combining.

v36's compound channel naïve weight 0.1. What if normalize each channel
to [0,1] per-patient before convex combination?
"""
...
def normalize_scores(score_dict):
    """Min-max normalize to [0,1] across diseases."""
    vals = list(score_dict.values())
    if not vals: return score_dict
...
def main():
    ...
    disease_full_phens = {}
    for d in dcs_list:
        disease_full_phens[d] = {p for _, p, ed in G.out_edges(d, data=True) if ed.get("etype")=="HAS_PHENOTYPE"} if d in G else set()
    ...
    with open("data/ddxplus/release_test_patients.csv") as f:
        for row in csv.DictReader(f):
            ...
            if args.normalize:
                s1_scores = normalize_scores(s1_scores)
                cov_scores = normalize_scores(cov_scores)
                prcov_scores = normalize_scores(prcov_scores)
                comp_scores = normalize_scores(comp_scores)

            final = {d: args.w_s1*s1_scores[d] + args.w_cov*cov_scores[d] + args.w_prcov*prcov_scores[d] + args.w_compound*comp_scores[d] for d in dcs_list}
    ...
    print(f"  normalize={args.normalize}: @1={100*c1/n:.2f}% @3={100*c3/n:.2f}% @5={100*c5/n:.2f}% @10={100*c10/n:.2f}% MRR={rr_sum/n:.4f}")
