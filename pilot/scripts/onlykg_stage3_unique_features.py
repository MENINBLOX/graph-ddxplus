#!/usr/bin/env python3
"""Stage 3: Top-K cluster-aware unique-feature reranking.

In top-K candidates, similar diseases share many phenotypes. The
DISCRIMINATING phenotypes are those UNIQUE to one disease within the cluster.

For each top-K candidate D:
  unique_phens(D | top_K) = phens(D) - union(phens(D') for D' in top_K, D' != D)
  unique_score(D | patient) = sum of weights for patient phens that are unique to D

Combine: Stage 1 + Stage 2 coverage + Stage 3 unique-feature.

Universal: computed entirely from KG structure + patient evidence.
"""
...
def main():
    ...
    ap.add_argument("--unique_weight", type=float, default=0.3)
    ...
    for d in dcs_list:
        ...
        d_all[d] = all_phens  # no hierarchy expansion for unique-feature
    ...
    with open("data/ddxplus/release_test_patients.csv") as f:
        for row in csv.DictReader(f):
            ...
            if ranked1[0] == true_cui: s1_only_c1 += 1
            ...
            # Stage 3: cluster-aware unique features within top_k
            # For each D in top_k, find phens UNIQUE among top_k (in ≤1 other disease)
            phen_count_in_topk = Counter()
            for d in top_k:
                for p in d_all.get(d, {}).keys():
                    phen_count_in_topk[p] += 1
            unique_scores = {}
            for d in top_k:
                d_phens = d_all.get(d, {})
                unique_match_w = 0.0
                for p, w in d_phens.items():
                    if phen_count_in_topk[p] == 1 and p in pcuis:
                        unique_match_w += w
                unique_scores[d] = unique_match_w

            # Normalize and combine
            max_s1 = max(s1_scores[d] for d in top_k) or 1
            max_cov = max(cov_scores.values()) or 1
            max_uniq = max(unique_scores.values()) or 1
            w_s1 = 1 - args.cov_weight - args.unique_weight
            combined = {}
            for d in top_k:
                s1_n = s1_scores[d] / abs(max_s1) if max_s1 != 0 else 0
                cov_n = cov_scores[d] / max_cov if max_cov != 0 else 0
                uniq_n = unique_scores[d] / max_uniq if max_uniq != 0 else 0
                combined[d] = w_s1 * s1_n + args.cov_weight * cov_n + args.unique_weight * uniq_n
    ...
    print(f"top_k={args.top_k} cov_w={args.cov_weight} uniq_w={args.unique_weight} ck={args.core_k} α={args.alpha}:")
    print(f"  S1-only @1: {100*s1_only_c1/n:.2f}%")
    print(f"  Final  @1={100*c1/n:.2f}% @3={100*c3/n:.2f}% @5={100*c5/n:.2f}% @10={100*c10/n:.2f}% MRR={rr_sum/n:.4f}")
