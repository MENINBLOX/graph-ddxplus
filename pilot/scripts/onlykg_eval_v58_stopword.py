#!/usr/bin/env python3
"""v58 KG-NB with global stopword filter on evidence CUIs.

Insight: evidence CUI degree distribution is heavy-tailed. Top ~244 CUIs (0.4%)
appear in 1000+ diseases — these are linguistic modifiers (Severe, Age, Pain)
extracted by scispaCy from text but provide zero discrimination.

Filter at KG-profile-build time: exclude evidence CUIs whose global in-degree > df_max.
"""
...
def compute_stopwords(G, df_max):
    """Find evidence CUIs with global in-degree > df_max."""
    in_deg = Counter()
    for _, p, ed in G.edges(data=True):
        if ed.get("etype") == "HAS_PHENOTYPE":
            in_deg[p] += 1
    stopwords = {c for c, d in in_deg.items() if d > df_max}
    return stopwords


def build_kg_profile_filtered(G, disease_cuis, mode, kappa, pr_fallback, stopwords):
    ...
    for d in disease_cuis:
        ...
        for _, p, ed in G.out_edges(d, data=True):
            ...
            if p in stopwords: continue  # GLOBAL STOPWORD FILTER
            cat = ed.get("category")
            if cat is None:
...
def main():
    ...
    ap.add_argument("--stopword_df_max", type=int, default=1000,
                    help="exclude evidence CUIs with global in-degree > this")
    args = ap.parse_args()
    ...
    stopwords = compute_stopwords(G, args.stopword_df_max)
    print(f"=== v58 stopword filter ===", flush=True)
    print(f"  stopword_df_max={args.stopword_df_max}, n_stopwords={len(stopwords)}", flush=True)

    kg_profile, all_evs = build_kg_profile_filtered(
        G, dcs_list, args.mode, args.kappa, pr_fallback, stopwords)
    log_prior = math.log(1.0 / len(dcs_list))

    print(f"  benchmark={args.benchmark}, mode={args.mode}, expand={args.expand_depth}",
          flush=True)
    print(f"  diseases={len(dcs_list)}, |all_evs|={len(all_evs)} (after stopword), patients={len(patients)}",
          flush=True)

    n = 0; c1=c3=c5=c10=0; rr_sum=0.0
    for true_cui, raw_pcuis in patients:
        if args.expand_depth > 0:
            patient_cuis = (expand_cuis(raw_pcuis, exp, args.expand_depth) - stopwords) & all_evs
        else:
            patient_cuis = (set(raw_pcuis) - stopwords) & all_evs
    ...
    print(f"v58 stopword {args.benchmark}"
          f"{('/'+args.rb_dataset) if args.benchmark=='rarebench' else ''} "
          f"mode={args.mode} df_max={args.stopword_df_max}: "
