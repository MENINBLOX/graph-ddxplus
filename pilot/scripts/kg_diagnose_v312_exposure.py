...
def main():
    ...
    # v312: epidemiologic exposure evidences identified as URTI/contagious-disease discriminators
    EXPOSURE_EVIDENCES = {"crowd", "contact", "dayc", "z77.22"}

    def patient_profile(evidences):
        pain_chars=[]; pain_locs=[]; pain_radiations=[]
        pain_intens=None; pain_speed=None; pain_present=False
        symptoms=[]; history=[]; exposure=[]
        for ev in evidences:
            ...
            # v312: route exposure-related evidences to a separate prominent section
            if base in EXPOSURE_EVIDENCES:
                exposure.append(q_clean + (f": {val_en}" if val_en else ""))
                continue
        ...
        if exposure:
            out.append("EXPOSURE/EPIDEMIOLOGIC: " + "; ".join(exposure)
                       + "\n  (epidemiologic exposure suggests contagious upper-respiratory infection: URTI/Influenza/Bronchiolitis/Viral pharyngitis)")
        return "\n".join(out)
