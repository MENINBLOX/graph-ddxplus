...
Candidate diagnoses with distinctive features:
{candidates}

Based on the patient's specific presentation, which diagnosis BEST matches?
...
def main():
    print("=" * 80, flush=True)
    print("진단 v18: prior + 감별 고유증상 + 구조화 re-ranking", flush=True)
    print("=" * 80, flush=True)
    ...
    for c in candidates:
        ...
        # 각 후보의 고유 증상 (다른 후보에 없는 것)
        topk = c["topk"]
        all_topk_syms = {}
        for dc in topk:
            all_topk_syms[dc] = set(ds.get(dc, {}).keys())

        cand_lines = []
        for i, dc in enumerate(topk):
            dname = cui2name.get(dc, cp.get(dc, dc))
            my_syms = all_topk_syms.get(dc, set())
            other_syms = set()
            for dc2 in topk:
                if dc2 != dc: other_syms |= all_topk_syms.get(dc2, set())
