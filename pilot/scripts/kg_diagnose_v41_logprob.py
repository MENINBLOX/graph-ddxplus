#!/usr/bin/env python3
"""진단 v41: LLM logprob-based ranking (next-token prediction).

가설: LLM의 next-token prediction은 multiple choice보다 정확.
각 후보 disease를 prompt 끝에 두고 LLM의 logprob 계산 → 가장 likely candidate 선택.

방법:
  prompt = patient profile + "Most likely diagnosis: " + disease_name
  prompt_logprobs로 disease_name 토큰의 logprob 합 계산
  최대 logprob의 candidate 선택
"""
...
DISEASE_NAME_FIX = {
    ...
    "HIV (initial infection)": "Acute HIV infection",
...
def main():
    print("="*80, flush=True)
    print("진단 v41: LLM logprob-based ranking", flush=True)
    print("="*80, flush=True)
    ...
    llm = LLM(model="google/gemma-4-E4B-it", dtype="bfloat16", max_model_len=2048,
              gpu_memory_utilization=0.70, enforce_eager=True,
              limit_mm_per_prompt={"image": 0, "audio": 0})

    # Logprob-based ranking
    print("\n[5] Logprob ranking...", flush=True)
    ...
    for c_idx, c in enumerate(candidates):
        ...
        base = f"Patient: {p['age']}yo {'Male' if p['sex']=='M' else 'Female'}\nChief complaint: {chief}\n{profile}\nMost likely diagnosis: "
        for d_idx, dc in enumerate(c["topk"]):
            name = disease_full_name(cui2name.get(dc, dc))
            full = base + name
            all_prompts.append(full)
            pair_meta.append((c_idx, d_idx, len(base)))

    print(f"  Total prompts: {len(all_prompts)}", flush=True)

    # Process in chunks to avoid OOM
    CHUNK = 2000
    outputs = []
    t0 = time.time()
    for chunk_start in range(0, len(all_prompts), CHUNK):
        chunk = all_prompts[chunk_start:chunk_start+CHUNK]
        chunk_outs = llm.generate(chunk, sampling)
        outputs.extend(chunk_outs)
        print(f"  진행: {chunk_start+len(chunk)}/{len(all_prompts)} ({time.time()-t0:.0f}초)", flush=True)
    print(f"  완료: {time.time()-t0:.0f}초", flush=True)

    # For each (c_idx, d_idx), compute logprob of disease_name tokens
    # outputs[i].prompt_logprobs is list of dicts (one per input token)
    # Each dict maps token_id -> Logprob obj
    scores = defaultdict(dict)  # c_idx -> {d_idx: score}

    tokenizer = llm.get_tokenizer()
    for i, out in enumerate(outputs):
        c_idx, d_idx, base_len = pair_meta[i]
        # Tokenize base and full to know boundary
        base_tokens = tokenizer.encode(all_prompts[i][:base_len])
        full_tokens = out.prompt_token_ids
        # disease tokens are full[len(base_tokens):]
        n_base = len(base_tokens)
        n_full = len(full_tokens)
        if n_full <= n_base:
            scores[c_idx][d_idx] = -1e9
            continue
        plp = out.prompt_logprobs  # list of len(prompt) elements
        if not plp:
            scores[c_idx][d_idx] = -1e9
            continue
        # logprob[j] is logprob of token[j] given tokens[<j]
        # We want sum of logprobs for tokens at positions [n_base, n_full)
        log_sum = 0
        cnt = 0
        for j in range(n_base, n_full):
            if j >= len(plp): break
            lp_dict = plp[j]
            if lp_dict is None: continue
            tok = full_tokens[j]
            if tok in lp_dict:
                log_sum += lp_dict[tok].logprob
                cnt += 1
        # Average to normalize for length
        scores[c_idx][d_idx] = log_sum / max(cnt, 1)

    # Rank candidates by logprob, evaluate
    t1 = t3 = t5 = 0
    for c_idx, c in enumerate(candidates):
        if c_idx not in scores:
            reranked = c["topk"]
        else:
            sc = scores[c_idx]
            sorted_indices = sorted(range(len(c["topk"])), key=lambda d_idx: -sc.get(d_idx, -1e9))
            reranked = [c["topk"][i] for i in sorted_indices]
        tdc = c["true_dc"]
    ...
    print(f"  v41 logprob ranked: @1={100*t1/nt:.1f}% @3={100*t3/nt:.1f}% @5={100*t5/nt:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v41 GTPA@1 = {100*t1/nt:.1f}% (SUBSET={nt})", flush=True)
