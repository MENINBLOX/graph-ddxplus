#!/usr/bin/env python3
"""진단 v86: v76 + TOP_K=16 features (더 많은 feature 제공).

v76 (TOP_K=8) → 62.4%
v82 (TF-IDF, removes generic) → 56.5% (너무 적음)
가설: TOP_K=16 raw count → 더 풍부한 매칭 단서, +1-3%p 기대.
"""
...
# Extended generic blacklist - filter only metadata noise, keep clinical signs
...
def main():
    print("="*80, flush=True)
    print("진단 v86: TOP_K=16 raw count features", flush=True)
    print("="*80, flush=True)
    ...
    for dc in dcs_list:
        feats = ds.get(dc, {})
        top_cuis = sorted(feats.items(), key=lambda x: -x[1])[:TOP_K_FEATURES * 4]
        names = []; seen = set()
    ...
    print(f"\n샘플:")
    for sample_name in ["URTI", "Pneumonia", "Sarcoidosis"]:
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
    ...
    np.save("pilot/results/v86_score_matrix.npy", score_matrix)
    ...
    print(f"  v86 (TOP_K=16) @1={100*t1/n:.1f}% @3={100*t3/n:.1f}% @5={100*t5/n:.1f}% @10={100*t10/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v86 GTPA@1 = {100*t1/n:.1f}% (SUBSET={n})", flush=True)
