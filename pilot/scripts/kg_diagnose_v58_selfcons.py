#!/usr/bin/env python3
"""진단 v58: self-consistency (temp=0.5, n=5 samples, average).
...
def main():
    ...
    # Self-consistency: temperature>0, multiple samples averaged
    sampling = SamplingParams(temperature=0.5, max_tokens=8, n=5)
...
How likely is this hypothesis given the patient's presentation? Reply with ONLY a percentage 0-100 (0=impossible, 100=certain)."""
    ...
    for chunk_start in range(0, len(convs), CHUNK):
        ...
        for i, out in enumerate(outs):
            ...
            # Average over multiple samples (self-consistency)
            scores = []
            for sample in out.outputs:
                m = re.search(r"(\d+)", sample.text.strip())
                if m:
                    s = min(int(m.group(1)), 100)
                    scores.append(s)
