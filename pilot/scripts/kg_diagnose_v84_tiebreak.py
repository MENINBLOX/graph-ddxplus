#!/usr/bin/env python3
"""진단 v84: v82 (TF-IDF) + tie-breaking for top-1 ties.

Stage 1: TF-IDF KG features × 49 → score matrix
Stage 2: For each patient with top-1 ties (max-tied >= 2), do a tie-break
         pairwise/listwise prompt asking LLM to pick THE single best.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v84: TF-IDF + tie-breaking", flush=True)
    print("="*80, flush=True)
    ...
    # Reuse v82 stage1 if exists
    stage1_path = Path("pilot/results/v82_score_matrix.npy")
    if stage1_path.exists() and np.load(stage1_path).shape[0] >= n:
        print(f"\n[Stage 1] 재사용: {stage1_path}", flush=True)
        score_matrix = np.load(stage1_path)[:n]
    else:
        print(f"\n[Stage 1] vLLM TF-IDF KG features 49-cand scoring...", flush=True)
...
How well does the patient's presentation match this diagnosis? Reply with ONLY a percentage 0-100 (0=no match, 100=textbook match)."""
        ...
        print(f"  Stage 1 LLM calls: {len(convs)}", flush=True)
        CHUNK = 5000
        ...
        np.save("pilot/results/v82_score_matrix.npy", score_matrix)
    ...
    print(f"\n[Stage 2] tie-breaking...", flush=True)
    tied_patients = []  # (c_idx, list of tied dc's)
    ...
    # Stat: how often is true_dc among the tied set
    ...
    # Listwise tie-break: LLM picks 1 from tied set
    tiebreak_sampling = SamplingParams(temperature=0, max_tokens=12)
    chosen_dc = {}  # c_idx → chosen dc
    tb_convs = []
    tb_meta = []  # (c_idx, [tied_dc1, tied_dc2, ...])
    for c_idx, tied_dcs in tied_patients:
        pd = patient_data[c_idx]
        # Build numbered list of tied diseases with their KG features
        lines = []
        for i, dc in enumerate(tied_dcs):
            name = disease_full_name(cui2name.get(dc, dc))
            kg_feats = disease_features[dc]
            lines.append(f"{i+1}. {name} — typical: {kg_feats}")
        disease_list = "\n".join(lines)
...
These diagnoses are equally likely so far. Pick the SINGLE most likely:
{disease_list}
...
Reply with ONLY the number of your choice."""
        ...
        for chunk_start in range(0, len(tb_convs), CHUNK):
            ...
            for i, out in enumerate(outs):
                ...
                if c_idx not in chosen_dc:
                    chosen_dc[c_idx] = tied_dcs[0]  # fallback
    ...
    for c_idx, c in enumerate(candidates):
        ...
        # If there's a tie at top-1, swap chosen_dc to position 1
        if c_idx in chosen_dc:
            chosen = dcs_list.index(chosen_dc[c_idx])
            if chosen in ranked[:len([i for i in ranked if score_matrix[c_idx, i] == score_matrix[c_idx, ranked[0]]])]:
    ...
    print(f"\n  v84 (TF-IDF + tie-break) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v84 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
