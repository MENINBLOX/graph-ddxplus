"""BOUND qualified-edge with SapBERT canonicalization of symptom & attribute-value
CUIs (patient value_cuis vocab vs disease scispaCy vocab don't share CUIs even for
same concept). Canonicalize -> bound matching fires. Random-sample search -> top3 full."""
...
CUI_ATTRS=["location","radiation","character"]
...
def suf(eid):
    ...
    return None
# ---- gather all symptom+attr CUIs to canonicalize ----
cset=set()
for d,phs in dz.items():
    for pc,at in phs:
        cset.add(pc)
        for a in CUI_ATTRS: cset|=at.get(a,set())
# patient side: all _question CUIs of parent symptoms + all location/rad/char value CUIs
...
print(f"canonicalize {len(cset)} CUIs",flush=True)
nm={}
...
for i in range(0,len(named),1024):
    sims=E[i:i+1024]@E.T
    idx=(sims>=THR).nonzero(as_tuple=False).cpu().numpy()
    for r,c in idx:
        gi=i+r
        if c>gi:
...
print(f"{len(named)} CUIs -> {len({find(i) for i in range(len(named))})} clusters (thr={THR})",flush=True)
# canonicalize disease bound
dzi={}
for d in dl:
    idx=defaultdict(lambda:defaultdict(set))
    for pc,at in dz.get(d,[]):
        cp=cz(pc)
        for a in CUI_ATTRS: idx[cp][a]|=czset(at.get(a,set()))
        for a in ("severity","onset","timing"): idx[cp][a]|=at.get(a,set())
    dzi[d]=idx
...
def eval_rows(rows,configs):
    acc={c:Counter() for c in configs}; n=0; bindhit=0
    for tc,evs in rows:
        ...
        for a in ATTRS:
            for d in dl:
                ...
                for sym in psym.values():
                    ...
                    if not sv: continue
                    for cp in sym["cuis"]:
                        if cp in idxd and idxd[cp].get(a): tot+=len(sv & idxd[cp][a])
                bb[a][d]=tot
            if a=="location": hit=sum(1 for v in bb[a].values() if v>0)
...
configs=[(s,lam) for s in subsets for lam in (0.02,0.05,0.1)]+[(frozenset(),0.0)]
print(f"random6000: {len(configs)} configs...",flush=True)
acc,n,bh=eval_rows(samp,configs)
res=sorted(configs,key=lambda c:-acc[c][1])
print(f"N={n} location-binding 발생 케이스={bh} ({100*bh/n:.1f}%)")
print(f"base@1={100*acc[(frozenset(),0.0)][1]/n:.2f} @10={100*acc[(frozenset(),0.0)][10]/n:.2f}")
...
accf,nf,_=eval_rows(allrows,top3+[(frozenset(),0.0)])
