#!/usr/bin/env python3
"""v69 — Evidence-level negative scoring.

v68 issue: a CUI can appear in BOTH pos and neg mappings via different
evidences (e.g. 'toux'=yes maps C0010200 cough; 'expecto'=no also maps
C0010200). Net signal is contaminated.

v69 fix: track each binary evidence as yes/no independently.
For each disease D, define expected_yes(ev) = how strongly D 'expects'
this evidence to be yes (= max edge weight in D's profile across the
evidence's CUIs).

  if patient answered ev = yes:
     contribution = expected_yes(ev) * idf_factor    # positive
  if patient answered ev = no:
     contribution = -(expected_yes(ev) * idf_factor) # negative
  if not asked:
     contribution = 0

Sum over all 208 binary evidences, normalize.
"""
...
def precompute_evidence_signal(profile, value_cuis, binary_evs, idf, beta):
    """For each disease D and binary evidence ev, signal(ev,D) =
    max weighted CUI in profile_D among ev's _question CUIs."""
    ev_cuis = {}
    for ev_id in binary_evs:
        m = value_cuis.get(ev_id, {})
        cuis = set(m.get("_question", []))
        ev_cuis[ev_id] = cuis
    signal = defaultdict(dict)  # signal[d][ev] = max prof[E] * idf^beta over E in ev_cuis
    for d, prof in profile.items():
        for ev, cuis in ev_cuis.items():
            best = 0.0
            for c in cuis:
                if c in prof:
                    val = prof[c]  # already idf-weighted
...
def load_ddxplus_full(n_max):
    ...
    with open("data/ddxplus/release_test_patients.csv") as f:
        for row in csv.DictReader(f):
            ...
            # Negative binary evidences
            neg_binary = binary_evs - answered_binary
            patients.append((true_cui, pos_pcuis, answered_binary, neg_binary)); n += 1
    return dcs_list, patients, binary_evs


def score_v69(pos_pcuis, neg_binary, profile, idf, beta, signal, lam):
    scores = {}
    ...
    for d, prof in profile.items():
        ...
        # Negative penalty: sum of expected_yes for evidences answered no
        sig = signal.get(d, {})
        neg_pen = sum(sig.get(ev, 0.0) for ev in neg_binary)
        # Normalize by sqrt of #neg or by d_norm
...
def evaluate(profile, idf, beta, signal, patients, all_evs, dcs_list, lam):
    ...
    for true_cui, pos_raw, _, neg_binary in patients:
        pos = pos_raw & all_evs
        if not pos: continue
        scores = score_v69(pos, neg_binary, profile, idf, beta, signal, lam)
...
def main():
    ...
    ap.add_argument("--lam_sweep", type=str, default="0.0,0.1,0.25,0.5,1.0,2.0,4.0")
    args = ap.parse_args()
    ...
    value_cuis = json.load(open(VALUE_CUIS))
    signal = precompute_evidence_signal(profile, value_cuis, binary_evs, idf, args.beta)

    avg_neg = sum(len(p[3]) for p in patients)/len(patients)
    avg_yes = sum(len(p[2]) for p in patients)/len(patients)
    print(f"=== v69 evidence-level negative — N={args.n} ===")
    print(f"  binary evidences: {len(binary_evs)}, avg yes-binary: {avg_yes:.1f}, avg no-binary: {avg_neg:.1f}")

    for lam in args.lam_sweep.split(","):
        lam = float(lam)
        r = evaluate(profile, idf, args.beta, signal, patients, all_evs, dcs_list, lam)
