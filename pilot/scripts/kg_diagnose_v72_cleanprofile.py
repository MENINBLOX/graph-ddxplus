#!/usr/bin/env python3
"""진단 v72: LLM이 환자 evidence를 임상 narrative로 cleanup → v54-style scoring.

가설: 환자 프로필의 원본 evidence dump가 노이즈 → LLM이 임상 정리한 narrative로
변환 후 scoring하면 신호↑.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v72: LLM-cleaned profile + v54 scoring", flush=True)
    print("="*80, flush=True)
    ...
    # Stage 1: LLM cleanup of patient profile (one call per patient)
    print("\n[4] Stage 1: LLM clinical narrative cleanup...", flush=True)
    sampling_clean = SamplingParams(temperature=0, max_tokens=200)
    cleanup_convs = []
    for c in candidates:
        p = c["patient"]
        ...
        prompt = f"""Patient questionnaire data:
Demographics: {age_sex}
Chief complaint: {chief}
{profile}

Convert this into a concise clinical case summary (3-5 sentences, professional medical language). Include only the most relevant findings. Output the summary text only, no preamble."""
        cleanup_convs.append([{"role": "user", "content": prompt}])

    print(f"  Cleanup calls: {len(cleanup_convs)}", flush=True)
    CHUNK = 5000
    cleaned = []
    t0 = time.time()
    for chunk_start in range(0, len(cleanup_convs), CHUNK):
        chunk = cleanup_convs[chunk_start:chunk_start+CHUNK]
        outs = llm.chat(chunk, sampling_clean)
        for out in outs:
            cleaned.append(out.outputs[0].text.strip())
        print(f"  진행: {chunk_start+len(chunk)}/{len(cleanup_convs)} ({time.time()-t0:.0f}초)", flush=True)
    print(f"  Stage 1 완료: {time.time()-t0:.0f}초", flush=True)
    # show first sample
    if cleaned: print(f"  샘플: {cleaned[0][:200]}", flush=True)

    # Stage 2: per-candidate scoring on cleaned narrative
    print("\n[5] Stage 2: Per-candidate scoring on cleaned narrative...", flush=True)
    sampling = SamplingParams(temperature=0, max_tokens=8)
    convs2 = []
    pair_meta2 = []
    for c_idx, c in enumerate(candidates):
        narrative = cleaned[c_idx]
...
How likely is this hypothesis given the case? Reply with ONLY a percentage 0-100 (0=impossible, 100=certain)."""
    ...
    for chunk_start in range(0, len(convs2), CHUNK):
        ...
        print(f"  진행: {chunk_start+len(chunk)}/{len(convs2)} ({time.time()-t0:.0f}초)", flush=True)
    print(f"  Stage 2 완료: {time.time()-t0:.0f}초", flush=True)

    np.save("pilot/results/v72_score_matrix.npy", score_matrix)

    # Pick top-1 from raw scores
    t1 = t3 = t5 = t10 = 0
    for c_idx, c in enumerate(candidates):
    ...
    print(f"\n  v72 (cleaned profile) @1={100*t1/n:.1f}% @3={100*t3/n:.1f}% @5={100*t5/n:.1f}% @10={100*t10/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v72 GTPA@1 = {100*t1/n:.1f}% (SUBSET={n})", flush=True)
