...
def main():
    ...
    LARGE_TIE_THRESHOLD = 3  # v307: limit to top-3 for deeper reasoning per candidate
    small_ties = [(c_idx, tdcs[:3]) for c_idx, tdcs in tied_patients if len(tdcs) <= 10]
    ...
    chosen_dc = {}
    cot_sampling = SamplingParams(temperature=0, max_tokens=600)  # v307: deeper reasoning per candidate
...
The following candidate diagnoses are tied. Reason carefully about each one in 3 sentences:
- Sentence 1: What patient evidence MATCHES this diagnosis?
- Sentence 2: What patient evidence is INCONSISTENT or contradicts this diagnosis?
- Sentence 3: Brief verdict (likely/unlikely) and why.

Then choose the candidate that has the strongest match and least contradiction.
...
ANALYSIS:
(1) MATCHES: <sentence>; INCONSISTENT: <sentence>; VERDICT: <sentence>
(2) MATCHES: <sentence>; INCONSISTENT: <sentence>; VERDICT: <sentence>
(3) MATCHES: <sentence>; INCONSISTENT: <sentence>; VERDICT: <sentence>
PICK: <number>"""
