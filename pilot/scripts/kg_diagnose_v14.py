...
def main():
    print("=" * 80, flush=True)
    print("진단 v14: 다중 알고리즘 union + 구조화 re-ranking", flush=True)
    print("=" * 80, flush=True)
    ...
    # [3] Multi-algo union top-15
    print("\n[3] 다중 알고리즘 union top-15...", flush=True)
    t0 = time.time()
    ...
    for p in patients:
        ...
        for dc in dcs:
            s = ds.get(dc, {})
            if not s: sc_b[dc] = -1e6; continue
            tw = sum(s.values()) + len(all_s) * 0.1
            sc_b[dc] = sum(math.log((s[x]+0.1)/tw+1e-10) if x in s else math.log(0.1/tw+1e-10) for x in ps)
        rb = sorted(sc_b.items(), key=lambda x: -x[1])

        # IDF
        sc_i = {}
        for dc in dcs:
            s = ds.get(dc, {})
            if not s: sc_i[dc] = 0; continue
            sc_i[dc] = sum((math.log(nd/(sdf[x]+1))+1)*c for x, c in s.items() if x in ps)
        ri = sorted(sc_i.items(), key=lambda x: -x[1])

        # v15
        sc_v = {}
        for dc in dcs:
            s = ds.get(dc, {})
            if not s: sc_v[dc] = 0; continue
            cf = sum(1 for x in s if x in ps)
            dn_ = sum(1 for x in s if x not in ps)
            sc_v[dc] = cf/(cf+dn_+1)*cf if cf else 0
        rv = sorted(sc_v.items(), key=lambda x: -x[1])

        # Union top-10 from each → deduplicate, keep bayesian order for ties
        union = set()
        for d, _ in rb[:10]: union.add(d)
        for d, _ in ri[:10]: union.add(d)
        for d, _ in rv[:10]: union.add(d)

        # Order by bayesian score
        topk = sorted(union, key=lambda d: sc_b.get(d, -1e6), reverse=True)[:15]
