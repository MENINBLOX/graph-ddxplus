...
def main():
    ...
    # v310: Self-consistency with N samples per tied case at temperature=0.5
    N_SAMPLES = 3
    chosen_dc = {}
    cot_sampling = SamplingParams(temperature=0.5, max_tokens=400, n=N_SAMPLES, seed=42)

    print(f"\n[Stage 2] Self-consistency CoT tie-break (N={N_SAMPLES})...", flush=True)
    tb_convs = []; tb_meta = []
...
PICK: <number>"""
    ...
    print(f"  Tie-break LLM calls: {len(tb_convs)} × {N_SAMPLES} samples = {len(tb_convs)*N_SAMPLES}", flush=True)
    CHUNK = 5000
    t2 = time.time()
    sample_outs = None
    for chunk_start in range(0, len(tb_convs), CHUNK):
        ...
        for i, out in enumerate(outs):
            ...
            if picks:
                ...
                pick_counts = _C(picks)
                top_pick = pick_counts.most_common(1)[0][0]
