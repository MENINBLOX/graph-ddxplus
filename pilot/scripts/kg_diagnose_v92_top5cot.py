#!/usr/bin/env python3
"""진단 v92: v79 stage1 → CoT pick from top-5 (always, not just ties).

Stage 1: 49 cand × patient (v79 stage1 reused)
Stage 2: Take top 5 by stage1 score → LLM CoT pick → final answer

Insight: 76.8% of non-tied wrong have true in top-5 → top-5 CoT can rescue.
Plus tie-break is naturally handled.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v92: v79 stage1 → CoT pick from top-5 (always)", flush=True)
    print("="*80, flush=True)
    ...
    # Take top-5 per patient
    TOP_N = 5
    top_per_patient = []
    for c_idx in range(n):
        ranked = np.argsort(-score_matrix[c_idx])
        top5 = []
        # Include all tied-at-top + next
        max_s = score_matrix[c_idx].max()
        for i in ranked:
            if score_matrix[c_idx, i] == max_s:
                top5.append(int(i))
            elif len(top5) < TOP_N:
                top5.append(int(i))
            if len(top5) >= max(TOP_N, (score_matrix[c_idx] == max_s).sum()):
                break
        top5 = top5[:8]  # cap at 8 for prompt sanity
        top_per_patient.append(top5)

    # Stat: how often is true in top-N
    in_topN = sum(1 for c_idx, c in enumerate(candidates)
                  if cui_to_idx[c["true_dc"]] in top_per_patient[c_idx])
    print(f"  True in top-{TOP_N}+: {in_topN}/{n} ({100*in_topN/n:.2f}%) [ceiling]", flush=True)
    ...
    print(f"\n[Stage 2] CoT top-5 pick...", flush=True)
    convs = []
    for c_idx, c in enumerate(candidates):
        pd = patient_data[c_idx]
        top_dcs = [dcs_list[i] for i in top_per_patient[c_idx]]
        lines = []
...
The following candidate diagnoses are most likely (preliminary screening). For each candidate, briefly evaluate (1-2 sentences) which patient features support or contradict it. Then pick the SINGLE most likely.
...
PICK: <number>"""
    ...
    for chunk_start in range(0, len(convs), CHUNK):
        ...
        for i, out in enumerate(outs):
            c_idx = chunk_start + i
            top_dcs = [dcs_list[idx] for idx in top_per_patient[c_idx]]
            ...
            if m:
                ...
                if 0 <= pick < len(top_dcs):
                    picked = top_dcs[pick]
            if picked is None:
                picked = top_dcs[0]  # fallback to stage1 top1
            final_pick[c_idx] = picked
    ...
    # Build final score matrix to compute @1, @3, @5
    final_score = score_matrix.copy()
    for c_idx, picked_dc in final_pick.items():
    ...
    print(f"\n  v92 (top-5 CoT) @1={100*t1c/n:.2f}% @3={100*t3c/n:.1f}% @5={100*t5c/n:.1f}% @10={100*t10c/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v92 GTPA@1 = {100*t1c/n:.2f}% (SUBSET={n})", flush=True)
