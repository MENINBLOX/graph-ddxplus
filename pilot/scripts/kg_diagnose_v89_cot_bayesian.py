#!/usr/bin/env python3
"""진단 v89: v87 (CoT tie-break) + Bayesian age/sex prior.

Apply Bayesian prior boost to:
A) Tie-breaking: when CoT can't decide, prefer high-prior diseases
B) Marginal gain on non-ties

CPU only post-process on v87 chosen_dc (re-runs CoT to get fresh chosen_dc and
saves it). Then sweep alpha for Bayesian.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v89: CoT tie-break + Bayesian prior", flush=True)
    print("="*80, flush=True)
    ...
    print("\n[Bayesian prior]: building from training data...", flush=True)
...
PICK: <number>"""
    ...
    for chunk_start in range(0, len(tb_convs), CHUNK):
        ...
        print(f"  CoT: {chunk_start+len(chunk:=tb_convs[chunk_start:chunk_start+CHUNK])}/{len(tb_convs)} ({time.time()-t2:.0f}초)", flush=True)

    # Build final score matrix combining v79 + tie-break boost
    final_score = score_matrix.copy()
    for c_idx in chosen_dc:
        chosen_idx = cui_to_idx[chosen_dc[c_idx]]
        max_s = score_matrix[c_idx].max()
        # Boost the chosen by tiny epsilon to break tie
        final_score[c_idx, chosen_idx] = max_s + 0.001

    # Baseline: CoT tie-break only
    t1_cot = sum(1 for c_idx, c in enumerate(candidates)
                 if dcs_list[int(np.argmax(final_score[c_idx]))] == c["true_dc"])
    print(f"\n  CoT tie-break (no Bayesian): @1={100*t1_cot/n:.2f}%", flush=True)

    # Sweep Bayesian alpha
    print("\n[Sweep] α (final = CoT_score + α * 100 * log_prior)")
    best_t1 = t1_cot; best_alpha = 0.0; best_M = final_score
    for alpha in [0.001, 0.01, 0.05, 0.1, 0.2, 0.3, 0.5, 1.0, 2.0]:
        combined = final_score + alpha * 100 * prior_log
        t1 = sum(1 for c_idx, c in enumerate(candidates)
                 if dcs_list[int(np.argmax(combined[c_idx]))] == c["true_dc"])
        print(f"  α={alpha:.3f}: @1={100*t1/n:.2f}%", flush=True)
        if t1 > best_t1:
            best_t1 = t1; best_alpha = alpha; best_M = combined

    t3 = sum(1 for c_idx, c in enumerate(candidates)
             if c["true_dc"] in [dcs_list[i] for i in np.argsort(-best_M[c_idx])[:3]])
    t5 = sum(1 for c_idx, c in enumerate(candidates)
             if c["true_dc"] in [dcs_list[i] for i in np.argsort(-best_M[c_idx])[:5]])
    print(f"\n  Best α={best_alpha}: @1={100*best_t1/n:.2f}% @3={100*t3/n:.1f}% @5={100*t5/n:.1f}%", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v89 GTPA@1 = {100*best_t1/n:.2f}% (α={best_alpha}, SUBSET={n})", flush=True)
