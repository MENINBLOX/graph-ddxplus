...
def main():
    ...
    print(f"[v315-epidcontext] Union: {n_medkg}/{len(disease_features)} have medkg, all have v87. Combined top-{TOP_K_FEATURES*2} per disease.")

    # v315: load per-disease epidemiology features
    epid_path = Path("/mnt/medkg/kg/disease_epidemiology_by_cui.json")
    epid_by_cui = json.load(epid_path.open()) if epid_path.exists() else {}
    print(f"[v315] Epidemiology features: {len(epid_by_cui)} diseases")
    ...
    for c_idx, tied_dcs in small_ties:
        ...
        for i, dc in enumerate(tied_dcs):
            ...
            epid = epid_by_cui.get(dc, [])
            block = f"({i+1}) {name} — typical features: {kg_feats}"
            if epid:
                block += f"\n    Epidemiology: {'; '.join(epid)}"
            lines.append(block)
        disease_list = "\n".join(lines)
