#!/usr/bin/env python3
"""v95 — Exhaustive LLM IE for rare diseases (RareBench coverage expansion).

Reuses v85 prompt template (benchmark-blind, exhaustive, generic medical few-shot).
Difference: loads rare disease pool from v95_rare_pool.json (pre-deduplicated,
Orphanet-named, excludes already-IE'd diseases from v85).

CLAUDE.md compliance:
- Principle 2: extends KG to cover RareBench (single KG for all benchmarks)
- Principle 5: benchmark-blind prompt (generic few-shot only)
- Principle 6: exhaustive, no phenotype count limit
"""
...
# Reuse v85 prompt verbatim (benchmark-blind, generic medical examples)
FEWSHOT = """\
...
def main():
    ...
    diseases = [(p["name"], "rarebench") for p in pool]
