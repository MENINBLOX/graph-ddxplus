...
files=sorted(glob.glob("pilot/data/cache/v104b_attr13_ie/*.json")); data=[json.load(open(f)) for f in files]
strs=set()
for o in data:
    for f in o["findings"]:
        strs.add(f["name"])
...
def patient(evs):
    ...
    return base,at
pickle.dump({"dz":dz,"s2c":s2c},open("pilot/data/cache/v104b_attr.pkl","wb"))
# === QUALITATIVE TRACE ===
print("\n"+"="*72)
...
print("\n빌드 완료: pilot/data/cache/v104b_attr.pkl")
