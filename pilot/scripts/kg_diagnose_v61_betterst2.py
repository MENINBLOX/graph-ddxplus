#!/usr/bin/env python3
"""진단 v61: v59 top-5 → CoT (rule out + reasoning) Stage 2.

가설: 10개 후보 중 1개 고르는 것보다 각각 점수 매기는 게 더 정확.
LLM이 "score 7" vs "score 3"으로 자연스럽게 표현.

각 (patient, candidate) 쌍 → "이 진단이 환자에 얼마나 맞는지 0-9?" → 점수.
모든 후보 점수 → 최고 점수 candidate 선택.
"""
...
def main():
    print("="*80, flush=True)
    print("진단 v50: per-candidate independent scoring", flush=True)
    print("="*80, flush=True)
    ...
    for p in patients:
        ...
        # v60: use ALL 49 candidates for Stage 1 (v59 approach)
        topk = list(dcs)
    ...
    # Per-candidate scoring: ask LLM to score each candidate independently
    print("\n[5] Per-candidate scoring prompts...", flush=True)
...
How likely is this hypothesis given the patient's presentation? Reply with ONLY a single digit 0-9 (0=very unlikely, 9=very likely)."""
    ...
    # Stage 1: Rank by score, take top-5
    print("\n[6] Stage 1: v50 score top-5 + Stage 2: list pick...", flush=True)
    stage1_top5 = []  # list of [dc1, dc2, ..., dc5]
    t1_v50 = 0
    for c_idx, c in enumerate(candidates):
        scored = [(d_idx, all_scores.get((c_idx, d_idx), 0)) for d_idx in range(len(c["topk"]))]
        scored.sort(key=lambda x: -x[1])
        ranked5 = [c["topk"][d_idx] for d_idx, _ in scored[:5]]
        stage1_top5.append(ranked5)
        if ranked5[0] == c["true_dc"]: t1_v50 += 1

    nt = len(candidates)
    print(f"  v50 stage1 @1={100*t1_v50/nt:.1f}%", flush=True)

    # Stage 2: list pick from top-5 with KG signatures
    sym_df = Counter()
    ...
    for dc, syms in ds.items():
        scored2 = [(s, c * idf.get(s, 0)) for s, c in syms.items() if sym_df.get(s, 99) <= DF_GATE]
        scored2.sort(key=lambda x: -x[1])
        sig = []
        for s, _ in scored2[:8]:
            name = cp.get(s, "")
            if name and len(name) > 3: sig.append(name)
            if len(sig) >= 5: break
        disease_signatures[dc] = sig

    sampling2 = SamplingParams(temperature=0, max_tokens=200)
    convs2 = []
    for c_idx, c in enumerate(candidates):
        p = c["patient"]
        profile = patient_profile(p["evidences"])
        ...
        chief = re.sub(r"Do you |Have you |Are you ", "", chief).rstrip("?").strip()
        cands_lines = []
        for i, dc in enumerate(stage1_top5[c_idx]):
            name = disease_full_name(cui2name.get(dc, dc))
...
5 DIAGNOSTIC HYPOTHESES (these have already been pre-screened as the most likely):
{cands}

Apply rigorous clinical reasoning:
1. For each candidate, identify ONE specific feature in the patient that supports it
2. For each candidate, identify ONE specific feature in the patient that ARGUES AGAINST it (or note "no contradictions")
3. Identify the candidate with the strongest support and weakest contradictions

Final answer (only the number 1-5):"""
        convs2.append([{"role": "user", "content": prompt}])

    print(f"  Stage 2 LLM calls: {len(convs2)}", flush=True)
    t0 = time.time()
    outputs2 = llm.chat(convs2, sampling2)
    print(f"  완료: {time.time()-t0:.0f}초", flush=True)

    t1 = t3 = pf = 0
    for c_idx, (c, out) in enumerate(zip(candidates, outputs2)):
        text = out.outputs[0].text.strip()
        # Look for "Final answer: N" or last digit in text
        m = re.search(r"[Ff]inal answer\s*:?\s*(\d)", text)
        if not m:
            # Fall back to last digit 1-5 in text
            nums = re.findall(r"\b([1-5])\b", text)
            if nums:
                idx_val = int(nums[-1])
                m = type('M', (), {'group': lambda self, x: str(idx_val)})()
        if m:
            idx = int(m.group(1)) - 1
            if 0 <= idx < 5:
                top5 = list(stage1_top5[c_idx])
                chosen = top5.pop(idx); top5.insert(0, chosen)
                reranked = top5
            else: reranked = stage1_top5[c_idx]; pf += 1
        else: reranked = stage1_top5[c_idx]; pf += 1
        tdc = c["true_dc"]
    ...
    print(f"  v50 stage1 @1={100*t1_v50/nt:.1f}%", flush=True)
    print(f"  v52 two-stage @1={100*t1/nt:.1f}% @3={100*t3/nt:.1f}%", flush=True)
    print(f"  parse_fail={pf}", flush=True)
    print(f"\n{'='*80}", flush=True)
    print(f"v52 GTPA@1 = {100*t1/nt:.1f}% (SUBSET={nt})", flush=True)
