...
**핵심 발견**: PubMed 공출현은 외형상 "노이즈"가 보이더라도 실제로는 LLM이 감별진단에 활용하는 광범위한 의학적 맥락(differential, complication, comorbid)을 포함하므로, 정제하거나 다른 소스로 대체할수록 오히려 성능이 떨어진다. SemMedDB의 typed relations는 단독으로는 너무 좁고, 결합해도 PubMed 단독 신호를 희석시킨다.
