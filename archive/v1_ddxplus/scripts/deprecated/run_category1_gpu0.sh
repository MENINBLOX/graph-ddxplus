#!/bin/bash
# Category 1: LLM only (GPU 0)

N_SAMPLES=${1:-27389}
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_DIR="/home/max/Graph-DDXPlus/results/cat1_${TIMESTAMP}"
mkdir -p "$OUTPUT_DIR"

echo "Category 1: LLM only (GPU 0)"
...
try:
    benchmark = VLLMBenchmark(
        model='$MODEL',
        mode='small_llm',  # Category 1: LLM only
