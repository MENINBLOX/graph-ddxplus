#!/bin/bash
# Re-run failed models for Category 2 (GPU 0)

N_SAMPLES=${1:-27389}
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_DIR="/home/max/Graph-DDXPlus/results/rerun_${TIMESTAMP}"
mkdir -p "$OUTPUT_DIR"

echo "Re-running failed models (Category 2, GPU 0)"
...
" 2>&1
    ...
    if [ -f "$RESULT_FILE" ]; then
        if grep -q '"gtpa_at_1"' "$RESULT_FILE"; then
            ...
            ERROR=$(python3 -c "import json; d=json.load(open('$RESULT_FILE')); print(d.get('error', 'Unknown')[:100])")
            echo "Error: $ERROR"
