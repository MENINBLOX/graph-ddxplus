#!/usr/bin/env python3
"""진단 타이밍 최적화 v2 - IL≈15 타겟, 더 많은 샘플."""
...
def main(n_cases: int = 100, target_il: float = 15.0):
    """최적화 실행."""
    optimizer = StoppingOptimizer()

    print("=" * 70)
    print(f"조기 종료 최적화 v2 (cases={n_cases}, target_IL={target_il})")
    print("=" * 70)

    # IL≈15 타겟에 맞춘 탐색 공간
    param_grid = {
        "min_il": [2, 3, 4],
        "confidence_threshold": [0.25, 0.28, 0.30, 0.32, 0.35],
        "gap_threshold": [0.08, 0.10, 0.12],
        "relative_gap_threshold": [1.8, 2.0, 2.2],
    }
    ...
    results_target = [r for r in results if abs(r["avg_il"] - target_il) <= 2]
    results_target.sort(key=lambda r: -r["gtpa_at_1"])

    print("\n" + "=" * 70)
    print(f"IL ≈ {target_il} (±2) 최적 결과")
    print("=" * 70)
    print(f"{'Config':<35} {'GTPA@1':>8} {'Top-3':>8} {'Top-5':>8} {'Avg IL':>8} {'Std':>6}")
    print("-" * 85)

    for r in results_target[:15]:
        print(f"{r['config']:<35} {r['gtpa_at_1']:>7.1%} {r['top_3_acc']:>7.1%} {r['top_5_acc']:>7.1%} {r['avg_il']:>7.1f} {r['std_il']:>5.1f}")

    # 전체 결과 (GTPA@1 순)
    results_by_acc = sorted(results, key=lambda r: (-r["gtpa_at_1"], r["avg_il"]))

    print("\n" + "=" * 70)
    print("GTPA@1 순 전체 결과")
    print("=" * 70)
    print(f"{'Config':<35} {'GTPA@1':>8} {'Top-3':>8} {'Avg IL':>8}")
    print("-" * 65)

    for r in results_by_acc[:15]:
        marker = " *" if abs(r["avg_il"] - target_il) <= 2 else ""
        print(f"{r['config']:<35} {r['gtpa_at_1']:>7.1%} {r['top_3_acc']:>7.1%} {r['avg_il']:>7.1f}{marker}")

    # 최적 결과 저장
    best = results_target[0] if results_target else results_by_acc[0]

    # 파라미터 파싱
    parts = best["config"].split("_")
    best_params = {
        "min_il": int(parts[0][3:]),
        "confidence_threshold": int(parts[1][1:]) / 100,
        "gap_threshold": int(parts[2][1:]) / 100,
        "relative_gap_threshold": int(parts[3][1:]) / 10,
    }
    ...
    output = {
        ...
        "best_result": best,
        "all_results_near_target": results_target[:20],
        "all_results_by_accuracy": results_by_acc[:20],
    }

    output_file = Path("results/stopping_optimization_v2.json")
    ...
    print("\n" + "=" * 70)
    print("최적 설정 (IL ≈ {})".format(target_il))
    print("=" * 70)
    print(f"""
min_il: {best_params['min_il']}
confidence_threshold: {best_params['confidence_threshold']}
gap_threshold: {best_params['gap_threshold']}
relative_gap_threshold: {best_params['relative_gap_threshold']}

GTPA@1: {best['gtpa_at_1']:.1%}
Top-3:  {best['top_3_acc']:.1%}
Top-5:  {best['top_5_acc']:.1%}
Avg IL: {best['avg_il']:.1f} (std: {best['std_il']:.1f})
""")
