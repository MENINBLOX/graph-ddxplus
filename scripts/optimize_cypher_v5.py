"""Cypher 쿼리 + Stopping 조건 종합 최적화 v5.

목표: IL ~15 달성
더 공격적인 stopping: 낮은 min_il, 낮은 confidence, 높은 ratio
"""
...
def main():
    print("=" * 70)
    print("Cypher + Stopping 종합 최적화 v5")
    print("목표: IL ~15 달성 (min_il=1, 더 공격적인 ratio)")
    print("=" * 70)
    print()
    ...
    # Grid search - 더 공격적인 조건
    denied_thresholds = [5]
    confidence_thresholds = [0.15, 0.18, 0.20]
    gap_thresholds = [0.04, 0.05, 0.06]
    relative_gaps = [1.3, 1.5, 1.8]
    min_ils = [1, 2]
    ...
    try:
        for i, config in enumerate(configs):
            print(f"[{i+1}/{len(configs)}] conf>={config.confidence_threshold}_gap>={config.gap_threshold}_ratio>={config.relative_gap}_min={config.min_il}", end=" ", flush=True)
            start = time.time()
        ...
        # Filter results with IL close to 15 (13-17 range) and sort by accuracy
        target_il_results = [r for r in results if 13 <= r['avg_il'] <= 17]
        target_il_results.sort(key=lambda x: -x['accuracy'])

        print()
        print("=" * 70)
        print("TOP 10 RESULTS (IL 13~17 범위, 정확도 순)")
        print("=" * 70)
        print()
        print(f"{'Conf':<8} {'Gap':<8} {'Ratio':<8} {'MinIL':<6} {'Accuracy':>10} {'Avg IL':>10} {'Exhausted':>10}")
        print("-" * 70)

        for i, r in enumerate(target_il_results[:10]):
            marker = "★" if i == 0 else " "
            print(f"{marker} >={r['confidence_threshold']:<5} >={r['gap_threshold']:<5} >={r['relative_gap']:<5} {r['min_il']:<6} {r['accuracy']*100:>9.1f}% {r['avg_il']:>10.1f} {r['exhausted_ratio']*100:>9.1f}%")

        if not target_il_results:
            print("No results in IL 13-17 range")
            # Show closest to IL 15
            by_il_diff = sorted(results, key=lambda x: abs(x['avg_il'] - 15))
            print()
            print("Closest to IL=15:")
            for r in by_il_diff[:5]:
                print(f"  conf>={r['confidence_threshold']}, gap>={r['gap_threshold']}, ratio>={r['relative_gap']}, min_il={r['min_il']} → Acc: {r['accuracy']*100:.1f}%, IL: {r['avg_il']:.1f}")

        # Also show best overall
        ...
        # Save all results
        output_file = Path("/home/max/Graph-DDXPlus/results/cypher_optimization_v5.json")
        with open(output_file, 'w') as f:
            json.dump(results, f, indent=2)
        print()
        print(f"All results saved to: {output_file}")

        # Best recommendation for IL ~15
        if target_il_results:
            best = target_il_results[0]
            print()
            print("=" * 70)
            print("추천 설정 (IL ~15 목표)")
            print("=" * 70)
