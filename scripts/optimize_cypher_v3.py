"""Cypher 쿼리 + Stopping 조건 종합 최적화 v3.

목표: IL ~15, Accuracy 최대화
다른 1,000개 케이스 사용 (offset 1000~2000)
"""
...
class OptimConfig:
    ...
    gap_threshold: float
    relative_gap: float = 1.8
    min_il: int = 2

    def __str__(self) -> str:
        return f"denied<{self.denied_threshold}_conf>={self.confidence_threshold}_gap>={self.gap_threshold}"
...
def main():
    print("=" * 70)
    print("Cypher + Stopping 종합 최적화 v3")
    print("목표: IL ~15, Accuracy 최대화")
    print("=" * 70)
    print()

    optimizer = CypherOptimizer()

    # Load patients (severity=2, offset 1000, count 1000) - 다른 케이스
    all_patients = optimizer.loader.load_patients(
        split="validate",
        n_samples=2000,
        severity=2,
    )
    patients = all_patients[1000:2000]  # 1000~2000 케이스 사용
    print(f"Using patients 1000-2000: {len(patients)} cases")
    print()

    # Grid search parameters
    denied_thresholds = [4, 5, 6, 7]
    confidence_thresholds = [0.30, 0.35, 0.40, 0.45]
    gap_thresholds = [0.08, 0.10, 0.12]

    configs = []
    for dt, ct, gt in product(denied_thresholds, confidence_thresholds, gap_thresholds):
    ...
    try:
        ...
        print("=" * 70)
        print()
        print(f"{'Denied':<8} {'Conf':<8} {'Gap':<8} {'Accuracy':>10} {'Avg IL':>10} {'Exhausted':>10}")
        print("-" * 60)

        for i, r in enumerate(target_il_results[:10]):
            marker = "★" if i == 0 else " "
            print(f"{marker}  <{r['denied_threshold']:<5} >={r['confidence_threshold']:<5} >={r['gap_threshold']:<5} {r['accuracy']*100:>9.1f}% {r['avg_il']:>10.1f} {r['exhausted_ratio']*100:>9.1f}%")
        ...
        # Save all results
        output_file = Path("/home/max/Graph-DDXPlus/results/cypher_optimization_v3.json")
