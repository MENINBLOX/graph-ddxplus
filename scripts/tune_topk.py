...
증상/진단 프롬프트에서 제시할 Top-K 후보 수의 최적값을 찾는 실험.

결과: 논문에서 Top-K 선택의 근거로 사용

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python scripts/tune_topk.py -n 500
"""
...
class TopKTuner:
    ...
    def run(self, n_samples: int, severity: int = 2) -> dict:
        ...
        print(f"\nTop-K={self.top_k}, Samples={len(patients):,}", flush=True)
        ...
        pbar = tqdm(total=len(states), desc=f"K={self.top_k}", unit="p")
...
Answer (1-{self.top_k}):"""
    ...
    def _build_diagnosis_prompt(self, state: PatientState, dx_candidates: list[tuple[str, str, float]]) -> str:
        ...
        # Top-K만 제시
        display_candidates = dx_candidates[:self.top_k] if dx_candidates else []
...
Answer (1-{min(self.top_k, len(display_candidates)) if display_candidates else 1}):"""
        return prompt
...
def run_topk_experiment(model: str, n_samples: int, k_values: list[int]) -> list[dict]:
    """Top-K 실험 실행."""
    results = []
    ...
    for k in k_values:
        ...
        tuner = TopKTuner(model=model, top_k=k)
...
def print_results(results: list[dict]):
    ...
    print("=" * 70)
    print(f"{'K':>4} {'GTPA@1':>10} {'Avg IL':>10} {'Time':>10}")
    print("-" * 70)
    ...
    for r in results:
        ...
        # Best: GTPA@1 최대화, IL 최소화 (가중 점수)
        score = gtpa - (il / 50) * 0.1  # IL 패널티
        if score > best_score:
            best_score = score
            best_k = k

        marker = " *" if k == best_k else ""
        print(f"{k:>4} {gtpa:>9.1%} {il:>10.1f} {time_min:>9.1f}m{marker}")

    print("-" * 70)
    print(f"* Best K = {best_k} (highest GTPA@1 with IL penalty)")

    # JSON 저장
    output_dir = Path(__file__).parent.parent / "results"
    output_dir.mkdir(exist_ok=True)
    output_file = output_dir / "topk_optimization.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Top-K Optimization Experiment")
    ...
    parser.add_argument("--k-values", type=str, default="1,2,3,4,5,6,8,10", help="K values to test (comma-separated)")
    ...
    results = run_topk_experiment(args.model, args.n_samples, k_values)
